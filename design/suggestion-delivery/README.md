...
`sample-contacts.js` holds the supplied email and E.164 mobile arrays in preferred order. `RecipientFields.jsx` uses the first entry by default and renders a dropdown only for multiple options. Introduction drafts select one email per person. Selections persist per person and channel until refresh. Both HTML exports use `recipient-reference-runtime.js` for the same behavior; regenerate both references after shared contact or style changes. The Sample Suggestion Data page documents the proposed v2 schema with `params.emails`, `params.mobile_phones`, and per-person introduction recipient arrays.
