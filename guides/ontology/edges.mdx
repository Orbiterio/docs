...
| `HAS_AWARD` | Game associated with a resolved ceremony/category/nominee entry; required non-empty `award_evidence_record_ids` plus the common envelope. [Complete node and edge example](/guides/open-work/video-game-waterfall#complete-award-node-and-game-edge) | Game recognition replay; the game is context for a named-person award, not a claim every contributor won |
...
**Finite queue branch:** the original coordinator may schedule a predeclared `series_evidence` stage for an admitted series, then freeze the combined company endpoint manifest and queue identity-only Company resolution followed by fact/graph replay. Direct series claims can yield several companies, but each reservation uses the original root and originating game's remaining limits. Shared Company identities and jobs deduplicate across series/games. Neither the series writer nor a Company leaf can admit another root, enumerate company catalogs or invoke another waterfall. [Main build rules and Counter-Strike example](/guides/open-work/video-game-waterfall#evidenced-series-developers).
...
The [complete example and its source records](/guides/open-work/video-game-waterfall#complete-award-node-and-game-edge) remain synchronized with the JSON fixture. No individual recipient edge is inferred for this game-only entry. Award artwork is a resolved game media reference, with no new artwork edge or upload.
