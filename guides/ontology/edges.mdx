...
A producer credit on a Broadway or West End production. Pattern-dependent with the music waterfall's `PRODUCED` (Artist → Recording, 18) — the target label disambiguates, the `INVESTED_IN` precedent. Stage productions extend the film/TV credit node family with a `stage: true` discriminator rather than a new label. Co-production pairs are derived through the production node, never written as `CO_PRODUCED_WITH`.

| | |
|---|---|
| **Pattern** | `(Person\|Company)-[PRODUCED]->(Film_TV {stage: true})` |
| **Weight** | **12 / 16 / 22 / 28 / 35** by billing tier |
| **Writer** | TBD — [Broadway co-producers lane](/guides/open-work/roadmap/broadway-co-producers) |

| `billing_tier` | Weight | Derived pair | Anchor |
|---|---|---|---|
| `lead_producer` | **12** | 24 | At `ADVISOR_TO`'s level (12), just ahead of `INVESTED_IN` (15) |
| `producer` | **16** | 32 | Below lead, above the check-writer tier |
| `co_producer` | **22** | 44 | ≈ angel round check (25); two random co-producers stay weakly linked |
| `associate_producer` / `executive_producer` | **28** | 56 | Moderate band, past-role territory |
| `in_association` | **35** | 70 | `NOMINATED_FOR`-grade |

**Description template:** `{Producer} produced {Title} ({billing tier}, {market}, {season}).`

| Property | Type | Req | Description |
|---|---|---|---|
| `billing_tier` | enum above | ✓ | Drives the weight |
| `billing_order` | integer | ✓ | 1-based position within the tier line |
| `couple_credit` | boolean |  | `true` on both spouse edges split from one couple token |
| `via_entity_uuid` | uuid |  | Set on person-level derivations when the credit is held by a shingle company |
| `market` | enum `broadway` \| `west_end` | ✓ | Mirrors the production |
| `mention_ref` | id | ✓ | Audit link to `stage_credit_mention` |
| `valid_from` / `valid_to` | date | ✓ / opt | Inherit production opening / closing |
| `source_urls`, `confidence` | | ✓ | Provenance \+ resolution outcome |

**Example** — the lane's own worked case (a real 2024 Broadway credit), values as the writer would set them:

```json
{
  "description": "Angelina Jolie produced The Outsiders (lead producer, Broadway, 2023-24).",
  "billing_tier": "lead_producer",
  "billing_order": 1,
  "market": "broadway",
...
| `RELEASED` | **10** |
| `ADVISOR_TO` | **12** |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>PRODUCED</span> (stage; Person\|Company → `Film_TV {stage}`) | **12 / 16 / 22 / 28 / 35** by billing tier |
| `WROTE_LYRICS` | **12** |
