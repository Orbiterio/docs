...
**Video game contract synchronized September 9, 2026:** [game edge directions, weights, properties, ownership and real examples](#video-game-edges), including the [complete DEVELOPED_BY contract](#developed-by). These are Go/GCP build contracts, not deployed game writers.
...
## Video game relationships — proposed Go contract

<Info>
**Baseline September 8, 2026; series developer extension locked September 9; game writers remain proposed.** This is the graph output reference for the [Go/GCP game waterfall](/guides/open-work/video-game-waterfall), with [node types and image examples](/guides/ontology/nodes#video-game-nodes). Every game-domain edge starts with `excluded_from_scoring: true`. Existing movie/music/publication edges keep their current behavior. Weights below are proposed endpoint-specific traversal values, not evidence of interpersonal closeness.
</Info>
...
Rows reserve endpoint contracts; a reused edge name does not authorize its existing writer to accept a new node type. Green names are proposed new families. All rows involving new game endpoints require Go type registration, fact election, endpoint readiness and the owning queued replay before activation.
...
| <span style={{ color: "#16a34a", fontWeight: 600 }}>DEVELOPED_BY</span> | Video_Game → Company | 35 | Named development credit; literal role and scope in relation records; no parent-company inference | Core |
...
| <span style={{ color: "#16a34a", fontWeight: 600 }}>BASED_ON</span> | Derivative → source, with at least one Video_Game endpoint | 55 | `source_property`, observed subject/object, normalized derivative/source QIDs, description evidence | Core cross-media bridge |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>SOUNDTRACK_FOR</span> | Release_Group or Music_Work → Video_Game | 55 | Explicit soundtrack/score relation and exact music identity; album and composition remain distinct | Music slice |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>USED_IN</span> | Recording → Video_Game | 60 | Exact recording use; optional scene, usage and game-version scope | Later music slice |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>IN_GAME_SERIES</span> | Video_Game → Game_Series | 75 | Typed series membership; ordinal when explicitly provided | Context slice |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>EDITION_OF</span> | Video_Game → Video_Game | 60 | Explicit edition/remake/remaster relation; `edition_kind` | Release slice |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>EXPANSION_OF</span> | Video_Game → Video_Game | 60 | Explicit expansion/DLC-to-base relation; separate verified identities | Release slice |
...
| <span style={{ color: "#16a34a", fontWeight: 600 }}>AVAILABLE_ON</span> | Video_Game or Game_Release → Game_Platform | 80 | Confirmed availability; retain release/version/date context | Optional context |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>USES_ENGINE</span> | Video_Game → Game_Engine | 80 | [Locked engine-use contract](#uses-engine): required use-record references and description; version strings only when evidenced | Optional context |
...
**Runtime gap:** the inspected Universe shared `edgeDescriptionConfig` currently selects this model with effort `max`; it does not yet satisfy the requested `low` setting. The game build must configure and validate low effort through the shared registry, including factuality, embedded-source-instruction rejection and non-empty output. This documentation update does not change or certify existing movie/music writers.
...
The older [theatre `ADAPTED_FROM` proposal](#stage-adapted-from) remains scoped to its non-game endpoints. A theatre work ↔ game relationship uses `BASED_ON`; it must not also be emitted as `ADAPTED_FROM`. Any broader name consolidation requires an explicit migration of non-game contracts. A production's already resolved underlying work can supply intake context without moving the adaptation edge or copying the production's cast to the game.
...
<Info>
**Status: specified in the v1.2 build plans (2026-08-21 cross-lane §3.6 sitting; `DONOR_TO` 2026-08-25), not deployed.** Folded onto this page 2026-09-05 so the ontology has one inventory; the lane pages remain the build specs — [Broadway co-producers](/guides/open-work/roadmap/broadway-co-producers), [Media co-occurrence](/guides/open-work/orbiter-univers-standalone/social-media-enrichment), [990 / FEC / Form D](/guides/open-work/roadmap/990-fec-form-d), [Donors](/guides/open-work/roadmap/donors). <span style={{ color: "#16a34a", fontWeight: 600 }}>Green</span> names follow the same convention as the section above: in the spec, no live writer.
</Info>
...
### <span style={{ color: "#16a34a", fontWeight: 600 }}>BASED_ON</span>
