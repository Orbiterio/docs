...
<Warning>
**Required scoring correction (Mark, September 22): lower weight = stronger; older work must have a HIGHER weight.** Current `AUTHORED` and `CONTRIBUTED_TO` code reverses that rule. Their values below report the bug, not the intended policy. Proposed reversal preserves the bands: project 15 →20 →25 →30 →35 →40 as age increases; publication 15 →18 →22 →25. Universe code and graph data remain unchanged pending developer implementation. [Developer change list](/files/graph/universe-developer-handoff.md).
</Warning>
...
| Music | `IMPRINT_OF` 12, `DISTRIBUTED_BY` 40 and `FROM_AREA` 70 have writers and observed edges | These are not forward gaps. `ACQUIRED_BY` 25 accepts Company → Company, beyond music labels. `SAME_AS` remains a legacy identity contract; the registry explicitly says it was not ported. |
...
| Appearances | Deep Bio writes `SPOKE_AT` and `APPEARED_IN`; both observed | The expanded media ledger/session contract remains proposed. `SESSION_OF` 30 is the accepted replacement for unbuilt media `PART_OF`; registry still has the older name. |
...
| Work recency | `AUTHORED`: age ≤1y =25, ≤3y =22, ≤6y =18, older =15; undated =25. `CONTRIBUTED_TO`: current/undated =40, then 35/30/25/20/15. | Current Go publication cutoffs differ from the old Xano snippet. Both formulas give older work a **lower traversal cost**; “newer = stronger” was incorrect. This is a confirmed implementation mismatch: required policy is older = higher cost. Developer correction is pending. |
...
**Implementation mismatch:** the historical/current code ladder below is reversed. Required policy is older work = higher weight. Proposed corrected ladder: current 15, then 20/25/30/35/40 with increasing age. See the [developer change list](/files/graph/universe-developer-handoff.md).
...
Person authored a publication-shaped work. **Current Go cutoffs:** age ≤1 year →25, ≤3 years →22, ≤6 years →18, older →15; undated →25. The Xano snippet below is historical and uses different boundaries. Both implementations currently assign lower cost to older work, **contrary to the required policy**. Developer correction is pending; this historical snippet must not be copied as the desired formula.
