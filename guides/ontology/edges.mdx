...
| **Writer** | TBD — Deep Bio ledger `people[].relationship = co_investor` |
...
| `funding_round_uuid` | uuid | When the round resolves to a `Funding_Round` node |
...
<Info>
  **Specification only — 2026-09-06; no theatre writer is built.** This replaces the earlier stage-as-`Film_TV` proposal. Endpoints follow [Node Types → Theatre](/guides/ontology/nodes#theatre-nodes); delivery follows the [Go build plan](/guides/open-work/roadmap/broadway-co-producers). The property examples below are synthetic contract examples, not records written to any graph.
</Info>
...
| `source_urls`, `source_names` | string[] · required, nonempty | Public supporting records and provider names for the selected claim |
...
**Example shared envelope** — synthetic UUIDs and timestamps; per-family fragments below supply/override `weight`, `description`, `layer`, `temporal_basis`, election reference and any extra properties. Example omissions are not a license to omit required envelope fields in implementation.
...
```json
{
  ...
  "source": "stage",
  "source_names": ["fixture_programme"],
  "source_urls": ["https://example.org/programmes/example-musical"],
  "evidence_refs": ["fixture:snapshot-1:credit-block-1"],
  "provenance_ref": "00000000-0000-4000-8000-000000000102",
  "projection_revision": 1,
  "policy_version": "stage-edges-proposal-v1",
  "observed_at": 1704067200000,
  "source_finality": "complete",
...
| `producer` | **16** | Generic producing credit; a source heading saying “Produced by” maps here |
...
This corrects the old worked example: the inspected IBDB generic producing block does not by itself establish a lead-producer credit. Billing is not evidence of an investment amount, ownership stake or individual service interval.
...
```json
{
  ...
  "description": "Example Producer is credited as a co-producer of Example Musical (Broadway)."
}
```
...
```json
{
  "chosen_fact_id": "00000000-0000-4000-8000-000000000201",
  "identity_basis": "reviewed_work_link",
  "work_version_ref": "Example licensed stage edition",
  "weight": 50,
  "layer": "evidence",
  "temporal_basis": "unknown",
  "description": "Example Musical's Broadway production stages the independently identified dramatic work Example Musical."
}
```
...
```json
{
  "association_id": "00000000-0000-4000-8000-000000000301",
...
```json
{
  "chosen_fact_id": "00000000-0000-4000-8000-000000000401",
...
**Synthetic property fragments** — each also includes the shared envelope and its own elected `chosen_fact_id` / `mention_ref`. The `edge_type` wrapper names the relationship; it is not an additional graph property.
...
[
  ...
  },
  {
    ...
    "properties": {
      "role": "composer",
      "source_role": "Music",
      "billed_name": "Example Composer",
      "weight": 8,
      "layer": "credit",
      "temporal_basis": "unknown",
      "description": "Example Composer is credited with composing the music for Example Musical."
    }
  },
  {
    ...
    "properties": {
      "role": "lyricist",
      "source_role": "Lyrics",
      "billed_name": "Example Lyricist",
      "weight": 12,
      "layer": "credit",
      "temporal_basis": "unknown",
      "description": "Example Lyricist is credited with the lyrics of Example Musical."
    }
  }
]
```
...
**Synthetic property fragments** — distinct hypothetical results, not three claims about the same award. Each also has the shared envelope and its own `award_ref` / `support_claim_ref`; named-party edges additionally have a resolved `recipient_ref`.
...
[
  {
    ...
    "properties": {
      ...
      "description": "Example Musical is the production named in this 2023 award result."
    }
  },
  {
    "edge_type": "WON_AWARD",
    "properties": {
      ...
      "layer": "recognition",
      "temporal_basis": "award_edition",
      "description": "Example Director is explicitly named as a winner in this 2023 directing award result."
    }
  },
  {
    ...
    "properties": {
      ...
      "layer": "recognition",
      "temporal_basis": "award_edition",
      "description": "Example Designer is explicitly named as a nominee in this 2023 design award result."
    }
  }
]
```
...
Owner/operator/lease claims stay in the shared-place ledger for now; this waterfall does not require new venue ownership edges. Cast, direction, touring lineage, individual performances, rights/licensing and investment edges need separate endpoint/evidence contracts before expansion. Reuse existing company `OFFICER_OF` / `BOARD_MEMBER_OF` only through the owning filings feature; none supplies a substitute stage-producing credit.
...
| Generic producing heading or corrected billing | Tier 16 for generic producing; changed tier updates the same edge, with old evidence retained |
...
A person's appearance in a podcast episode, YouTube video, or conference session — the `MediaAppearance` node introduced by the media lane and shared with the podcast lane. Weight is set by the relationship class the appearance implies: an interview binds host and guest tightly (`COLLABORATORS`-class), a panel loosely, a conference hardly at all (a hub).
...
| **Writer** | TBD — [Media co-occurrence lane](/guides/open-work/roadmap/media-co-occurrence) |

| Relationship class | Derivation | Weight per edge | Pair |
|---|---|---|---|
| Interview host ↔ guest | Two `APPEARED_IN` on one interview appearance | **15** (`COLLABORATORS`-grade per edge) | 30 — two 15s through a degree-3 episode node, no hub risk |
| Co-panelists, same session | Two `APPEARED_IN {role: panelist}` on one session | **20** | 40 (`CO_NOMINATED_WITH`-grade) |
| Co-speakers, same conference | Shared `Event` via `PART_OF` | **45** at hub-scale events (speaker count ≥ threshold, default 50); **15** below it | evidence only (the `PERFORMED_AT` analogue). Same rule as [`SPOKE_AT`](#spoke_at), reconciled 2026-09-05 |

**Description template:** `{Person} appeared in {title} as {role}.` — `"…on {show}"` appended for podcast episodes.
...
**Deep Bio mapping (decided 2026-09-05).** The Deep Bio ledger's `works[].type = podcast` rows (3 in the first batch: two guests, one host) are written as this edge: `role` from the row, weight **15** (an episode is an interview-class appearance), target `MediaAppearance {source: "podcast"}` keyed on the episode URL (`episode_ref`), title and year from the row, and the ledger's `source_urls`, `evidence` and `mention_ref` as provenance. The interim ledger name `APPEARED_ON` is retired; the gate vocabulary in orbiter-universe `deepbio/edges.go` follows in a code change.

**Example** — real Deep Bio ledger row, 2026-09-05 batch, shown as an `APPEARED_IN` edge:

```json
{
  "description": "Will Misselbrook appeared in \"How Great Brands Earn Your Attention\" as guest, on FocusMatters.",
  "role": "guest",
  "weight": 15,
  "confidence": "confirmed",
  "source_urls": ["https://podcasts.apple.com/us/podcast/focusmatters/id1844946598", "https://www.iheart.com/podcast/1333-focusmatters-299422657/"],
  "evidence": [
    "Noah and Will explore the dynamics of attention in the marketing and sports industries.",
    "How Great Brands Earn Your Attention with Will Misselbrook. January 6, 2026 • 66 mins."
  ],
  "source_published": ["2026-01-06", "2026-01-06"],
  "mention_ref": "deep_bio:01a070ef-9842-7495-b6a8-5d070869925c:works[0]",
  "_target": { "label": "MediaAppearance", "source": "podcast", "title": "How Great Brands Earn Your Attention", "valid_from": "2026-01-06", "episode_ref": "https://podcasts.apple.com/us/podcast/focusmatters/id1844946598" }
}
```

---

### <span style={{ color: "#16a34a", fontWeight: 600 }}>PART_OF</span>

Structural containment: a conference session belongs to its conference. Exists so session-level and conference-level co-occurrence stay distinguishable at query time. Traversal weight **30**, the `RECORDING_OF` analogue; no band. The media lane sketched a minimal `Event` node for the target — that folds into the shipped `Event:Live_*` family ([node model](/guides/ontology/nodes#live_event)), which already carries `event_type`, dates, venue and city. Pattern-distinct from the reserved user-graph project `PART_OF`.

| | |
|---|---|
| **Pattern** | `(MediaAppearance)-[PART_OF]->(Event:Live_*)` |
| **Weight** | **30** |
| **Writer** | TBD — [Media co-occurrence lane](/guides/open-work/roadmap/media-co-occurrence) |

**Description template:** `{Session title} was a session at {Event name} ({year}).`
...
Two people who co-appear in news coverage over a rolling window. **Evidence layer, excluded from scoring** — co-mention is corroboration, not closeness. Written once per pair in canonical uuid order, updated by the daily batch, retired when the window drops the pair below floor.

| | |
|---|---|
| **Pattern** | `(Person)-[CO_MENTIONED_WITH]->(Person)` — symmetric, once per pair |
| **Weight** | **55** *(traversal only; `excluded_from_scoring: true`)* |
| **Writer** | TBD — [Media co-occurrence lane](/guides/open-work/roadmap/media-co-occurrence) |

**Description template:** `{A} and {B} co-appear in news coverage ({article_count} articles, {first_seen}–{last_seen}).`

| Property | Type | Req | Description |
|---|---|---|---|
| `article_count` | integer | ✓ | Distinct articles in the window |
...
| `layer` | `evidence` | ✓ | |
| `excluded_from_scoring` | boolean `true` | ✓ | Hard flag scoring respects |
...
| <span style={{ color: "#16a34a", fontWeight: 600 }}>APPEARED_IN</span> (Person → `MediaAppearance`) | **15** interview · **20** panelist · **45** conference speaker (15 below the hub threshold) |
...
| <span style={{ color: "#16a34a", fontWeight: 600 }}>PART_OF</span> (`MediaAppearance` → `Event:Live_*`) | **30** |
