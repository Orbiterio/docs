...
**Video game contract synchronized September 8, 2026:** [game edge directions, weights, properties, ownership and real examples](#video-game-edges). These are Go/GCP build contracts, not deployed game writers.
...
<Info>
**Synchronized September 8, 2026; game writers remain proposed.** This is the graph output reference for the [Go/GCP game waterfall](/guides/open-work/video-game-waterfall), with [node types and image examples](/guides/ontology/nodes#video-game-nodes). Every game-domain edge starts with `excluded_from_scoring: true`. Existing movie/music/publication edges keep their current behavior. Weights below are proposed endpoint-specific traversal values, not evidence of interpersonal closeness.
</Info>
...
| <span style={{ color: "#16a34a", fontWeight: 600 }}>USES_ENGINE</span> | Video_Game → Game_Engine | 80 | Explicit engine and version evidence | Optional context |
| `HAS_AWARD` — extension | Video_Game → Game_Award | 40 | Scoped award body/category/ceremony/nominee entry; separate same-game recipient entries; outcome is mutable | Awards slice |
...
| `DEVELOPED_BY` | Exact developer or explicitly described co-development/port-development role; `development_role`, correlated role/scope-record IDs | Game company replay; parent ownership or a generic “supporting” company flag does not establish development |
...
| `HAS_AWARD` | Game associated with a resolved ceremony/category/nominee entry; award-evidence record IDs | Game recognition replay; the game is context for a named-person award, not a claim every contributor won |
...
**Availability projection:** a release → platform edge describes that exact release. A game → platform edge is a deduplicated summary supported by explicit game availability and/or elected released scopes of the same canonical game. Both use the same owner; the summary is not a second source assertion or a second unit of scoring support. Never aggregate a separately identified remake into its original. If its final eligible scope is withdrawn, retire the summary edge. A surviving historical release supports “released on”; current store sale status is a separate time-scoped fact, so delisting does not erase release history.
...
| `description`, `description_generated_at` | Evidence-grounded description; BASED_ON uses the plan's 25–45-word sentence convention |
