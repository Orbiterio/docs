...
| `PUBLISHED_BY` | Explicit publisher with scoped territory/platform/time records | Game company replay; distribution, rights ownership and unlabeled company billing stay distinct |
| `BASED_ON` | Explicit derivative/source relation, preserving original P144/P4969 direction and qualifiers | Shared adaptation replay; never infer from shared franchise, sequel order or Best Adaptation category alone |
| `SOUNDTRACK_FOR` / `USED_IN` | Exact music endpoint plus game relation; soundtrack/work/recording kind and scoped usage records | Music/game bridge replay; named composer or an album track list alone cannot establish the bridge |
...
| `EDITION_OF` | Separately identified derivative game and exact source-game remake/remaster/edition claim; `edition_kind`, relation-record IDs | Game version replay; orient derivative → original. Packaging-only SKU, port date, sequel and similarity do not authorize this edge |
| `EXPANSION_OF` | Substantive game/expansion identity with explicit base-game relation; `expansion_kind`, scoped relation-record IDs | Game version replay; reject soundtrack/cosmetic apps and do not follow the base game's catalog |
| `RELEASE_OF` | Resolved `Game_Release` scope with one canonical game target; release-scope record IDs | Game release replay; exactly one active game target, no date-only release identity |
...
| `WON_AWARD` / `NOMINATED_FOR` | Explicit independently resolved Person/Company recipient of that particular entry; recipient-record IDs | Game recognition replay; billing an organization next to a game does not make it an award recipient |
...
| `evidence_ids` | string[] | Required. Immutable evidence references; Wikidata statement IDs identify assertions whose snapshots/revisions remain in the ledger |
