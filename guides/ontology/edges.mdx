...
**Current audit: September 22, 2026.** The [current Universe reference](#current-universe-writers-and-parameters) below supersedes historical implementation/status claims. Game writers now exist; [game contracts](#video-game-edges) and [DEVELOPED_BY](#developed-by) retain the detailed evidence requirements.

The historical core entries on this page were generated from the then-live `util.template_engine` (and `api.lambda`-built) Cypher blocks inside Xano workspace 3. Compact enrichment inventories, such as MusicBrainz, point to the deeper pipeline-specific implementation pages. Entries include:
...
**Weight convention:** Lower numbers indicate stronger / more important relationships. Weighted edges use the integer 1–100 scale; explicitly unweighted meta/user-content edges omit `weight`. Identity bridges use 1 and hub damping uses 100 — the IMDB film/TV edges' old sub-1 weights were raised to 30–40 by the §3.6 audit (2026-05-31); never write sub-1 weights.

**Status convention:** implementation is established by writer code; graph presence is established by a dated observation. Neither a registry `live` flag nor absence from one graph proves deployment everywhere. Green names mark remaining proposals; historical proposal text is retained where it defines a broader contract than the implemented writer.

**Required description policy — September 9, 2026:** every edge must have a non-empty, evidence-grounded `description`. Reuse suitable existing/source text; when missing, generate with **GLM 5.3 FLASH (`z-ai/glm-5.3-flash`), reasoning effort `low`**, before creating the edge. Queue generation in queued waterfalls and retain actual provenance; failed generation leaves the relation pending. Historical writer extracts below describe their audited behavior and may predate this requirement; they do not establish compliance. See the [creation and description contract](#required-description-and-queued-creation) for the game implementation and the current receipt behavior below.

<Note>
  **Legacy extracts:** the Xano function IDs and quoted Cypher below are historical references, last audited July 9, 2026; they are not the current Go parameter schema. Theatre is partially implemented. Use the September 22 writer audit and endpoint-specific status notes before copying an old example. Synthetic contract examples are not observed graph records.
</Note>


## Current Universe writers and parameters

Verified against [`orbiter-universe` revision `bb0fc045`](https://github.com/Orbiterio/orbiter-universe/tree/bb0fc045) and read-only aggregate queries against the graph configured by **dev Universe AlloyDB**, September 22, 2026. This did not inspect every environment or re-audit App/user-graph writers.

The graph contained **24,755 edges across 56 relationship types**. Of those, **24,333 carry weights**; the **422 `HAS_INVESTMENT_THESIS` edges are intentionally unweighted**. **6,304 edges have missing/blank descriptions**; current writer policy does not retroactively repair existing data. All observed weighted edges are within 1–100. These counts include retained records regardless of `active`/scoring eligibility and are not counts of currently traversable relationships.

[Current weights and endpoint variants](/guides/ontology/edge-weights#current-universe-reference) · [Weights CSV](/files/graph/edge-weights.csv) · [Observed property coverage CSV](/files/graph/edge-properties-observed.csv) · [Dated aggregate evidence](/files/graph/graph-audit-2026-09-22.json)

<Warning>
**Scoring rule (Mark, September 22): lower weight = stronger; older work has a HIGHER weight.** The Universe registry (`internal/kernel/edges`) was corrected on 2026-09-22: `CONTRIBUTED_TO` grades **15 / 20 / 25 / 30 / 35 / 40** and `AUTHORED` **15 / 18 / 22 / 25** as age increases, an undated work takes the weakest tier, and the age is measured against an injected clock instead of a fixed 2026. The graph counts on this page are the **pre-fix snapshot**: edges already stored still carry the old values until the bounded reconciliation run lands. [Developer change list](/files/graph/universe-developer-handoff.md).
</Warning>

### Corrected implementation differences

| Area | Current writer / observed behavior | Difference from older text or registry |
| --- | --- | --- |
| Employment | `WORKS_AT` current seniority 5–20; `WORKED_AT` past seniority +20 (25–40); `WAS_ADVISOR_TO` 12 | Go uses distinct past-tense labels. Observed `WORKS_AT` still spans 5–40, so historical rows are not uniformly relabeled. |
| Direct angel investment | Person:Angel → Company `INVESTED_IN` 25 | The former work-derived 15 branch is defunct; funding-round variants remain VC 20 / angel 25. |
| Music | `IMPRINT_OF` 12, `DISTRIBUTED_BY` 40 and `FROM_AREA` 70 have writers and observed edges | These are not forward gaps. `ACQUIRED_BY` 25 accepts Company → Company, beyond music labels. `SAME_AS` remains a legacy identity contract; the registry note reads "legacy Xano writer only, Universe never writes it". |
| Game relationships | `MergeGameEdge` implements the registered endpoint allowlist; `MergeBasedOn` owns adaptation | Game relationships are no longer blanket “unbuilt.” The snapshot includes `DEVELOPED_BY`, `IN_GAME_SERIES`, `USES_ENGINE` and game publishing edges, but not every supported family. |
| Theatre | `MergeProduced`: Person/Company → Stage_Production, 12/16/22/28/35. `MergeStageWorkedOn`: Person → Stage_Production, 30. Stage awards 40/30/35. | Registry corrected 2026-09-22: `edges.ProducedStage` targets `Stage_Production`, status live, and the stage lane reads its weights from it; `edges.StageWorkedOn` 30 is in `edges.All()`. Award recipient writer currently accepts **Person only**, not the proposed Company extension. No theatre edges were observed in this snapshot; implementation is not proof of deployed data. |
| Person relations | Deep Bio person-relation writer implements the documented fixed/tiered costs; several families observed | The whole section is no longer “not deployed.” Missing families in this snapshot do not prove missing writers. |
| Appearances | Deep Bio writes `SPOKE_AT` and `APPEARED_IN`; both observed | The expanded media ledger/session contract remains proposed. `SESSION_OF` 30 is the accepted replacement for unbuilt media `PART_OF`; the registry carries `edges.SessionOf` (planned) since 2026-09-22. |
| Governance / donations | `OFFICER_OF` and `DONOR_TO` have writers; only `OFFICER_OF` observed here | Do not label the entire v1.2 lane set unbuilt. `POLITICAL_DONOR_TO` / `RELATED_PERSON_OF` remain proposals in the registry. |
| Work recency | Corrected 2026-09-22. `AUTHORED`: age ≤1y =15, ≤3y =18, ≤6y =22, older =25; undated =25. `CONTRIBUTED_TO`: ongoing or ending this year =15, then 20/25/30/35 for 1–4 years, 40 older; finished with no end year =40. | Older work now costs more, as required. Go publication cutoffs (≤1/≤3/≤6) are kept; the old Xano snippet used current/≤2/≤4. Undated is the weakest tier because missing evidence is not recent evidence. Age is measured against an injected clock (`person.Deps.Now`), so an edge re-grades on every re-enrichment. Stored edges keep the old values until the reconciliation run. |

Source files: [`edges.go`](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/kernel/edges/edges.go), [`person_edges.go`](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/graph/person_edges.go), [`music_edges.go`](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/graph/music_edges.go), [`game_edges.go`](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/graph/game_edges.go), [`stage.go`](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/graph/stage.go), [`stage_award.go`](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/graph/stage_award.go), [`appearances.go`](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/graph/appearances.go).

### Current property envelope and family-specific parameters

There is **no universal flat edge schema**. Property names, optionality, merge keys and date formats vary by writer. The observed-property CSV reports **presence counts by type**, not requiredness or validity, and can combine several endpoint variants of the same type.

| Fields | Current meaning / scope |
| --- | --- |
| `uuid`, `created_at`, `updated_at` | Stable relationship identity and writer timestamps. Do not recreate the UUID or creation timestamp on retry. Graph `timestamp()` and receipt timestamps use epoch milliseconds. |
| `weight` | Endpoint-specific traversal cost. Omitted intentionally on `HAS_INVESTMENT_THESIS`; never interpret omission as zero. Some writers set weight only on creation (music credits, stage `WORKED_ON`); changing the registry alone does not reweight existing edges. |
| `description`, `description_generated_at` | Non-empty evidence-grounded prose plus actual receipt time in the shared verified-description path. A timestamp alone does not prove LLM generation. |
| `description_provenance`, `description_input_hash` | Shared writer stores the serialized receipt and input hash. Receipt includes provenance/model/prompt/verification data; do not invent those fields in examples. |
| `description_source`, `description_upgrade_pending` | Distinguishes verified generated prose from an accepted-relation fallback. Current code can persist `accepted_relation_fallback` with upgrade pending and **no model claim**. This differs from the stricter proposed rule that model failure always holds the edge pending. |
| `source` / `provider` | Writer-specific lane/provider names, not interchangeable universal keys. Deep Bio uses `provider`; games use `source=video_game`; stage uses `source=stage`. |
| `source_urls`, `source_names`, `evidence`, `evidence_refs`, `source_published` | Deep Bio person-relation/appearance provenance. `valid_from` / `valid_to` there are optional **integer years**. Other families use strings or different temporal fields; do not universally convert them to timestamps. |
| `relationship`, `via_key`, `via_entity_uuid`, `via_organization`, `via_domain`, `funding_round_uuid` | Person-relation scope. Organization-scoped merges include `via_key`; symmetric pairs are oriented lower UUID first. Optional fields depend on retained evidence. |
| `evidence_ids`, `chosen_fact_ids`, `source_urls`, `projection_revision`, `policy_version`, `active`, `layer`, `excluded_from_scoring` | Game envelope: requires elected evidence and resolved endpoints; `policy_version=games-v1`, `layer=evidence`, scoring excluded. Extra family properties are strings/string lists; preserve correlated source claims relationally. |
| `role`, `active`, `stage_credit_version`, `source_name`, `source_role`, `market` | Stage `PRODUCED`: endpoint-pair merge, revision-fenced updates. `WORKED_ON` instead keys on pair + `role` + `character_name`, with `billing`, `award_wins` and `award_nominations` as applicable. |

The [shared description writer](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/graph/edge_descriptions.go) requires a nonempty description and input hash before these guarded writes; the [registry](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/llm/registry.go) pins dedicated edge-description LLM calls to **`z-ai/glm-5.3-flash`, reasoning `low`**. The earlier `max` mismatch is resolved in this checkout. [Fallback receipt logic](https://github.com/Orbiterio/orbiter-universe/blob/bb0fc045/internal/platform/llm/edge_fallback.go) records when the output is deterministic fallback rather than model-generated. This is a code inspection, not a claim that all stored edges have migrated to the receipt contract.

### Keeping this reference current

Refresh the registry-derived CSV and table with `scripts/sync-universe-edge-reference.py`; instructions are in `files/graph/README.md` in the docs repository. Review writer exceptions and accepted proposals when source changes. Keep a separately dated read-only graph snapshot: local code, registry status and observed graph data answer different questions. No graph writes, reweighting or description backfill are part of this documentation update.
...
<a id="invested_in-person-company" />
...
| **Full schema reference** | [Known — MENTOR edge](/guides/known/schema-design/edges) |
...
**Corrected 2026-09-22 (Mark's rule: older work = higher weight).** The Universe writer grades **15** for an ongoing project or one ending this year, then **20 / 25 / 30 / 35** for one to four years since the end, **40** for five or more, and **40** for a finished project with no end year. The Xano snippet below is historical and reversed; do not copy its ladder.
...
| **Weight** | **dynamic: 15 / 20 / 25 / 30 / 35 / 40** by years since the project ended (`edges.ContributedTo`) |
| **Writer** | Universe: `enrichment/person/steps_credentials.go` → `platform/graph/person_credentials.go` `MergeCredentialEdge`. Historical: `mvp/resolve/resolve-edges-projects-publications` (#12579) |
...
| Recency (Universe, 2026-09-22) | Weight |
|---|---|
| Ongoing (`is_current`), or end year ≥ this year | **15** |
| Ended 1 year ago | **20** |
| Ended 2 years ago | **25** |
| Ended 3 years ago | **30** |
| Ended 4 years ago | **35** |
| Ended 5+ years ago | **40** |
| Finished, no end year (`undated`) | **40** |

The historical Xano ladder above (40 → 15 as age increases) is the reversed one and is kept only as a record.

---

### AUTHORED

Person authored a publication-shaped work. **Corrected 2026-09-22 (Mark's rule: older work = higher weight).** Universe cutoffs: age ≤1 year → **15**, ≤3 years → **18**, ≤6 years → **22**, older → **25**; undated → **25** (the weakest tier: missing evidence is not recent evidence). The Xano snippet below is historical, reversed and uses different boundaries; do not copy it.

| | |
|---|---|
| **Pattern** | `(Person)-[AUTHORED]->(Work:Publication)` |
| **Weight** | **dynamic: 15 / 18 / 22 / 25** by years since publication (`edges.Authored`) |
| **Writer** | Universe: `enrichment/person/steps_credentials.go` → `platform/graph/person_credentials.go` `MergeCredentialEdge`. Historical: `mvp/resolve/resolve-edges-projects-publications` (#12579) |
...
**Proposed game endpoint extension:** `Video_Game → PUBLISHED_BY → Company` uses weight **45** and game publishing scope under the [game contract](#game-published-by). The publication variant below retains weight **20** and its current owner. An edge-type-only global weight replacement is invalid.
...
The [Go game endpoint extension](#video-game-edges) reuses `Person → WORKED_ON → Video_Game` at weight **30**, preserving all roles in one person/game edge. It has its own bounded game credit replay and does not change the movie writer below.
...
<a id="video-game-edges" />

<a id="video-game-relationships--proposed-go-contract" />

## Video game relationships — Go contract

<Info>
**Baseline September 8–9; writer implementation verified September 22, 2026.** `MergeGameEdge` and the shared `MergeBasedOn` now implement the registered endpoint contracts. Graph presence is reported separately in the current audit. This is the graph output reference for the [Go/GCP game waterfall](/guides/open-work/video-game-waterfall), with [node types and image examples](/guides/ontology/nodes#video-game-nodes). Every game-domain edge starts with `excluded_from_scoring: true`. Existing movie/music/publication edges keep their current behavior. Weights below are proposed endpoint-specific traversal values, not evidence of interpersonal closeness.
</Info>

**Movie-credit handoff:** a stored person credit for a game queues the [same existing game identity into the game waterfall](/guides/open-work/video-game-waterfall#movie-to-game-handoff). Retain each role/character assertion and reconcile existing `WORKED_ON` credits through one elected game replay owner. The trigger is durable upstream evidence; creating or replaying the graph edge must not launch discovery. Pending game nodes and their incident edges remain internal until the [game publication gate](/guides/open-work/video-game-waterfall#game-publication-gate) succeeds; public readers enforce readiness on both endpoints.

**Person-role intake:** the [person pipeline adapter](/guides/open-work/video-game-waterfall#person-role-entry) uses job titles to queue a bounded search for named game credits. Title matches and employment authorize a probe only; explicit elected person/game evidence is still required for `WORKED_ON`. Esports, recruiting or marketing relevance does not imply development work.

**Music entry:** the [music-to-game adapter](/guides/open-work/video-game-waterfall#music-waterfall-entry) lands evidence before queued game admission. `Release_Group` / `Music_Work → SOUNDTRACK_FOR → Video_Game` and `Recording → USED_IN → Video_Game` preserve exact music scope. Artist/album credits are not copied into game `WORKED_ON`; shared title, soundtrack classification or track membership alone cannot establish a bridge.

**Official awards:** [The Game Awards adapter](/guides/open-work/video-game-waterfall#the-game-awards-adapter) feeds game recognition and named recipient edges. Each recipient links to their own nominee entry; a shared game or billed company does not make everyone a winner. Performance/music participation is a separate elected credit claim.

### Game edge inventory

Rows describe the game endpoint contracts implemented by the Go writer allowlist. A reused name still requires the correct endpoint-specific writer, elected evidence and endpoint readiness; implementation does not imply every family is populated in every graph.

| Edge | Direction | Weight | Edge-specific evidence / properties | Delivery |
| --- | --- | ---: | --- | --- |
| `WORKED_ON` — extension | Person → Video_Game | 30 | `roles`, `role_labels`, `character_names`, `credit_record_ids`, `credit_coverage`; keep language/platform/edition scope | Core |
| DEVELOPED_BY | Video_Game → Company | 35 | Named development credit; literal role and scope in relation records; no parent-company inference | Core |
| `DEVELOPED_BY` — series endpoint extension | Game_Series → Company | 35 | Elected direct series developer claim or an elected member development credit joined to that exact member's elected series membership; preserve the basis and member scope | Required when evidenced |
| `PUBLISHED_BY` — extension | Video_Game → Company | 45 | Named publisher; correlated territory/platform/date records; no distribution-to-publishing inference | Core |
| BASED_ON | Derivative → source, with at least one Video_Game endpoint | 55 | `source_property`, observed subject/object, normalized derivative/source QIDs, description evidence | Core cross-media bridge |
| SOUNDTRACK_FOR | Release_Group or Music_Work → Video_Game | 55 | Explicit soundtrack/score relation and exact music identity; album and composition remain distinct | Music slice |
| USED_IN | Recording → Video_Game | 60 | Exact recording use; optional scene, usage and game-version scope | Later music slice |
| IN_GAME_SERIES | Video_Game → Game_Series | 75 | Typed series membership; ordinal when explicitly provided | Context slice |
| EDITION_OF | Video_Game → Video_Game | 60 | Explicit edition/remake/remaster relation; `edition_kind` | Release slice |
| EXPANSION_OF | Video_Game → Video_Game | 60 | Explicit expansion/DLC-to-base relation; separate verified identities | Release slice |
| `RELEASE_OF` — extension | Game_Release → Video_Game | 30 | Exact release identity and scoped platform/territory/edition; distinct dispatch from music Release → Release_Group | Release slice |
| AVAILABLE_ON | Video_Game or Game_Release → Game_Platform | 80 | Confirmed availability; retain release/version/date context | Optional context |
| USES_ENGINE | Video_Game → Game_Engine | 80 | [Locked engine-use contract](#uses-engine): required use-record references and description; version strings only when evidenced | Optional context |
| `HAS_AWARD` — extension | Video_Game → Game_Award | 40 | [Locked recognition contract](#game-has-award): scoped award-evidence records, consistent current outcome and required description; separate same-game recipient entries | Awards slice |
| `WON_AWARD` — extension | Explicit Person or Company → Game_Award:Winner | 30 | Named recipient evidence, independent of the game's award | Awards slice |
| `NOMINATED_FOR` — extension | Explicit Person or Company → Game_Award:Nominee | 35 | Named nominee evidence; no credit-list fan-out | Awards slice |

Existing music-owned `RELEASED`, `COMPOSED`, `PERFORMED` and related edges remain music outputs with their existing weights; existing theatre-owned `WROTE` remains theatre context. `HAS_EXPERTISE` uses the existing taxonomy/policy when the follow-on slice is enabled. No new Person, Company or Music_Artist relationship identities are invented for games. No `KNOWS` clique, inferred employment or award-to-everyone edges are emitted.

<a id="game-published-by" />

<a id="game-edge-contract-v1" />

### Locked game relationship contract — games-v1

**Build handoff:** all fifteen families below need writer/description/evidence/correction/public-read coverage. The [phase guide](/guides/open-work/video-game-waterfall-build-guide) and [eval catalog](/assets/video-game-waterfall/build-evals.json) assign those gates. Older simulation rows are not automatically complete golden writer inputs; phase 0 supplies missing scoped references from retained evidence without changing its facts.

**Design baseline locked September 8, 2026.** The inventory above fixes names, directions and weights; this table fixes admission and ownership. Runtime writers remain proposed. [Source coverage](/guides/open-work/video-game-waterfall-sources) and [six node contracts](/guides/ontology/nodes#game-type-contract-v1) must pass before an endpoint extension activates. The existing endpoint-specific weights stay unchanged. **Every family in this table carries the [common envelope](#common-game-edge-properties-and-corrections) unchanged, including `source="video_game"` as the writing lane — also for `BASED_ON`, whose shared adaptation replay owner still writes the game lane whenever a `Video_Game` endpoint is present; the evidence provider (`wikidata`, `musicbrainz`, …) is retained in the source ledger, never in `source`. `policy_version` is `games-v1`; `games-v1-proposed` in older examples is pre-lock text (Phase 0b OQ-1/OQ-3, September 9, 2026).**

| Relationship | Accepted evidence and additional properties | Single replay owner / rejection rule |
| --- | --- | --- |
| `WORKED_ON` | Explicit named person/game credit from retained IMDb, typed Wikidata, official credit or enabled licensed source; `roles`, `role_labels`, `credit_record_ids`, optional `character_names`, language and version scope | Game credit replay; role title, employment, award proximity or name-only match cannot create the credit |
| `DEVELOPED_BY` | Video_Game → Company: exact development/co-development/port-development role. Game_Series → Company: direct series developer evidence or exact elected member development plus membership. Preserve `development_role`, `company_role_record_ids`; series variants also carry `development_scopes`, `series_developer_record_ids` and member references when applicable | One game company replay owner for both endpoint variants. Accepted evidence requires Company identity creation/reuse and edge replay; parent ownership, publisher billing and names copied from a card cannot establish development |
| `PUBLISHED_BY` | Explicit publisher with scoped territory/platform/time records; `publishing_record_ids` | Game company replay; distribution, rights ownership and unlabeled company billing stay distinct |
| `BASED_ON` | Explicit derivative/source relation, preserving original P144/P4969 direction and qualifiers; `adaptation_record_ids` | Shared adaptation replay; never infer from shared franchise, sequel order or Best Adaptation category alone |
| `SOUNDTRACK_FOR` / `USED_IN` | Exact music endpoint plus game relation; scoped usage records as `usage_record_ids`; `SOUNDTRACK_FOR` names the soundtrack/work/recording kind as `soundtrack_kind`, `USED_IN` its optional scene/usage/game-version scope as `usage_scope` (OQ-8) | Music/game bridge replay; named composer or an album track list alone cannot establish the bridge |
| `IN_GAME_SERIES` | Typed video-game series membership; `membership_record_ids`, optional literal `series_ordinal` | Game context replay; reject generic franchise/group membership and title similarity |
| `EDITION_OF` | Separately identified derivative game and exact source-game remake/remaster/edition claim; `edition_kind`, `relation_record_ids` | Game version replay; orient derivative → original. Packaging-only SKU, port date, sequel and similarity do not authorize this edge |
| `EXPANSION_OF` | Substantive game/expansion identity with explicit base-game relation; `expansion_kind`, `relation_record_ids` | Game version replay; reject soundtrack/cosmetic apps and do not follow the base game's catalog |
| `RELEASE_OF` | Resolved `Game_Release` scope with one canonical game target; `release_scope_record_ids` | Game release replay; exactly one active game target, no date-only release identity |
| `AVAILABLE_ON` | Exact platform plus verified released/availability scope; `availability_record_ids`, optional `release_entity_ids`, territory/date summaries | Game release/context replay; announced, backward-compatible or cloud-streamed alone is insufficient. Scoped SQL records are authoritative |
| `USES_ENGINE` | Explicit engine use on the exact game; `engine_use_record_ids`, optional `engine_versions`, scoped release/platform/time references | Game context replay; do not infer from a studio's tools, a trailer or middleware usage |
| `HAS_AWARD` | Game associated with a resolved ceremony/category/nominee entry; required non-empty `award_evidence_record_ids` plus the common envelope. [Complete node and edge example](/guides/archive/video-game-build-plan#complete-award-node-and-game-edge) | Game recognition replay; the game is context for a named-person award, not a claim every contributor won |
| `WON_AWARD` / `NOMINATED_FOR` | Explicit independently resolved Person/Company recipient of that particular entry; `recipient_record_ids` | Game recognition replay; billing an organization next to a game does not make it an award recipient |

These owner names designate modules/stages in the **shared Go application and queue**, not new services. They consume elected facts and bounded manifests. A writer cannot trigger source discovery, full person/company enrichment, an award archive or another game's waterfall. Related games needed for version/adaptation endpoints remain hidden identity-only stubs unless already independently admitted or published.

**Edge identity and cardinality:** one semantic edge per canonical ordered endpoint pair and edge family, shared across supporting sources. Multiple roles, engine versions, publishing regions or release observations become correlated SQL records referenced by that edge; they do not become duplicate parallel edges or Cartesian products. `RELEASE_OF` additionally enforces one active parent per release. Merges rekey through the canonical resolver; stale aliases cannot resurrect an obsolete endpoint. Reject game-to-itself adaptation/edition/expansion claims after alias resolution; a second alias does not create a second work.

<a id="developed-by" />

### DEVELOPED_BY

**Authoritative property and creation contract — September 9, 2026.** The game build plan and machine-readable contract must follow this entry. Status: proposed Go/GCP writer; documenting an edge does not mean it exists in production.

| Contract | Required behavior |
| --- | --- |
| Allowed direction | `Video_Game → DEVELOPED_BY → Company` or `Game_Series → DEVELOPED_BY → Company` |
| Meaning | The specific company has elected development involvement in that game or series. A series edge does not establish development of every member |
| Owner | `game_company` fact/graph replay in the shared Go application |
| Identity | One stable semantic edge per edge family and canonical ordered endpoint pair, shared by all supporting sources and roles |
| Weight / scoring | `weight=35`, `layer="evidence"`, `excluded_from_scoring=true` |
| Company creation | Reuse the canonical identity or queue a minimal `Entity:Company` identity when admitted evidence resolves a missing organization. Do not substitute its parent |

#### DEVELOPED_BY properties

The graph contains primitive scalars and lists. Evidence records with correlated qualifiers live in AlloyDB. Required lists must be non-empty for an active edge; optional empty properties are omitted. Names and QIDs in the example's endpoint wrappers are review aids, not relationship properties or replacements for canonical UUIDs.

| Property | Type | Requirement and meaning |
| --- | --- | --- |
| `uuid` | string / UUID | Required. Resolve or allocate once through the shared semantic relation registry; preserve across retries and source additions |
| `created_at` | integer, epoch milliseconds | Required. Set once on first materialization, never from the source's publication/release date |
| `updated_at` | integer, epoch milliseconds | Required. Application-managed time of an accepted material change; a no-op retry preserves it |
| `source` | string | Required: `video_game`, the writing lane. `game_company` is its replay owner; actual evidence providers are retained in the source ledger and URLs |
| `source_urls` | string[] | Required. URLs for the surviving eligible evidence, pinned to a source revision where available |
| `evidence_ids` | string[] | Required. Immutable evidence references; Wikidata statement IDs identify assertions whose snapshots/revisions remain in the ledger. A statement ID retained only inside a correlated record is **not** evidence until its claims snapshot is in the ledger; until then the edge stays pending-snapshot (Phase 0b OQ-4) |
| `chosen_fact_ids` | string[] | Required. Elected fact IDs supporting this exact relationship, including both developer and membership facts when using member support |
| `projection_revision` | integer | Required. Monotonically accepted revision, enforced by the shared fenced replay so older work cannot resurrect a retired relation |
| `policy_version` | string | Required. Pinned version of the policy that elected the materialized facts |
| `active` | boolean | Required. `true` only while eligible support survives; retained retired records use `false` and are excluded from public traversal |
| `weight` | integer | Required: `35` |
| `layer` | string | Required: `evidence` |
| `excluded_from_scoring` | boolean | Required: `true` for both endpoint variants |
| `description` | string | Required, non-empty and evidence-grounded **before a new edge is written**. Reuse suitable text; otherwise generate with GLM 5.3 FLASH at low effort under the description rule below |
| `description_generated_at` | integer, epoch milliseconds | Required when writing the validated description. Preserve with unchanged text/input; the description ledger distinguishes source reuse from model generation |
| `development_role` | string | Optional lossless role summary. P178 can support `developer`, not an invented lead/co-development/porting role. If surviving scopes have different roles, omit this scalar rather than choosing one; retain all roles in their records |
| `company_role_record_ids` | string[] | Required for a game edge and for series member support. References exact game/company development records, preserving any evidenced platform, edition, territory and time qualifications |
| `development_scopes` | string[] | Required only for series edges. Sorted unique values: `series_explicit`, `member_credit`, or both |
| `series_developer_record_ids` | string[] | Required only for series edges. Each record binds this series/company pair to a direct statement or to the same member's development **and** membership records |
| `supporting_game_ids` | string[] / UUIDs | Required when series support includes `member_credit`. Canonical IDs of those exact admitted members; do not expose unpublished member support |
| `membership_record_ids` | string[] | Required when series support includes `member_credit`. Elected memberships for those same games in this exact series |

**Conditional fields:** a direct series-only claim needs `development_scopes=["series_explicit"]` and `series_developer_record_ids`; it does not manufacture member/game-role records. A member-supported series edge needs all five scope/reference arrays. A game edge needs `company_role_record_ids` and omits the series-only arrays. A mixed series edge retains both bases, with each basis correlated in its own ledger record; arrays must never be cross-joined into new facts. Do not copy game/series descriptions, images, `developers` arrays, company logos or model configuration onto the edge.

#### Required description and queued creation

**Description rule:** every game edge requires a non-empty factual `description`. Reuse an existing or source-provided sentence only if it still describes the elected endpoints and scope. Otherwise queue `describe` using **`z-ai/glm-5.3-flash`**, **`ReasoningEffortNested: "low"`** (`reasoning: {"effort": "low"}`). The model sees retained eligible facts only; it cannot create evidence or discover endpoints. Keep the shared provider routing, usage accounting, prompt validation, attempt logging and original root limits. No blank edge, fabricated description receipt or silent model substitution on failure. Existing edge-specific length rules still apply; the Game_Series node's 127-character limit is not an edge limit.

**Runtime update — September 22, 2026:** `edgeDescriptionConfig` now uses `z-ai/glm-5.3-flash` with nested effort `low`. The guarded graph writer requires a nonempty verification receipt. Current fallback handling may provide a deterministic accepted-relationship sentence marked `description_upgrade_pending=true`; it does not claim LLM generation. See the [current property envelope](#current-property-envelope-and-family-specific-parameters). Existing blank-description edges remain a separately measured data gap.

1. **Retain and elect evidence.** Land the exact source assertion, revision, provider, qualifiers and entitlement/coverage outcome. Accept explicit development evidence; for member-derived series support require the developer credit and membership of the **same** admitted game. Publisher billing, employment, ownership and a display name alone cannot pass election.
2. **Reserve the bounded work.** Only the original root coordinator admits endpoints and predeclared later jobs. Freeze the combined Company manifest and atomically reserve existing root/per-game limits with durable job/outbox records. The series evidence stage emits claims, never its own jobs.
3. **Resolve endpoints.** Reuse canonical series/game and Company UUIDs; queue `identity_only` for a missing, resolvable Company. Conflicting identities remain pending. Recheck canonical aliases before commit. No full Company waterfall, company catalog or member-game enumeration.
4. **Build the elected relation.** `game_company` gathers surviving scoped records under the canonical pair, with one relation UUID and desired projection revision. It queues required later work through the coordinator. Company leaves and graph writes never trigger discovery.
5. **Prepare the description.** Reuse valid evidence-grounded text or run the reserved `describe` job. Validate against the desired endpoints, evidence and revision; retain actual model, effort, prompt/input hash and output provenance in the description ledger. Failure leaves `description_pending` or the applicable failure/budget reason and blocks first edge materialization. Never use a description as proof that an edge exists.
6. **Replay, then publish.** The shared parameterized graph writer consumes the elected payload and validated description, preserving UUID/creation time and enforcing the current revision/lease fence. Record graph and display receipts before declaring completion. Public output requires `active=true`, both endpoints ready and, for member support, the supporting game's publication. The series `developers` list updates from these eligible associations under the same projection revision; a name-only write is not completion.
7. **Correct without recursion.** Retries resume the same relation and budget. Re-election recomputes records, fields and text from surviving support; an invalidated description hides the affected projection until repaired. Final support withdrawal retires the edge and projected developer name without waiting for a model call, preserving the Company and unrelated edges. Partial coverage cannot retract omitted claims. Use the [shared Universe failure/stage/attempt logging](/guides/open-work/video-game-waterfall#shared-universe-error-logging-required); no new game-only error vocabulary.

The finite order is `retained source evidence → identity_only / fact election → describe if needed → graph_replay → publish`. For series, `series_evidence` precedes Company resolution. All steps retain the original root, manifest and generation. A graph may contain cycles; this dependency chain cannot recurse.

#### Zelda → Nintendo example

Wikidata's exact series item **The Legend of Zelda (`Q12393`)** has developer **Nintendo (`Q8093`)**, P178 statement `Q12393$cb44d861-42e7-52b9-da05-eb61a8767e64`, in [revision 2540158156](https://www.wikidata.org/w/index.php?title=Q12393&oldid=2540158156). Its rank is normal and it has no attached references; it is a source assertion to elect under the pinned policy, not independent corroboration or proof that Nintendo developed every title. P123 publisher evidence is not used for this edge.

**Expected-output simulation:** real names/QIDs/source statement; illustrative internal UUIDs, fact election, timestamps, readiness, revisions and description. No Company/edge creation or model call was performed. The example assumes the claim passes election and both endpoints are ready. [Download the example and correlated evidence record](/assets/video-game-waterfall/developed-by-example.json).

```json
{
  "from": "157afbee-f522-5c79-bab7-43069324c867",
  "from_ref": "Q12393",
  "type": "DEVELOPED_BY",
  "to": "7447fa80-dcff-540e-8b2d-01a0180bce2e",
  "to_ref": "Q8093",
  "properties": {
    "uuid": "018c5ca7-3d51-5df3-a87f-191bf011e413",
    "created_at": 1788955200000,
    "updated_at": 1788955200000,
    "source": "video_game",
    "source_urls": [
      "https://www.wikidata.org/w/index.php?title=Q12393&oldid=2540158156"
    ],
    "evidence_ids": [
      "Q12393$cb44d861-42e7-52b9-da05-eb61a8767e64"
    ],
    "chosen_fact_ids": ["7ce873a1-191b-54a7-a2d2-5f997743dafd"],
    ...
    "description": "Nintendo is credited as a developer of The Legend of Zelda series.",
    "description_generated_at": 1788955200000,
    "development_role": "developer",
    "development_scopes": ["series_explicit"],
    "series_developer_record_ids": ["0e734ee1-4ca8-52ff-98b6-67482624db67"]
  }
}
```

If Nintendo already exists, reuse it; otherwise the admitted evidence requires one queued minimal Company identity and this one series edge. It adds `Nintendo` to the series' sorted developer projection and creates no individual Zelda game credits. Member-derived examples instead require the exact member's development and membership records, as specified above.

<a id="game-series-developers" />

#### Series developer evidence

**Series developer creation is required when evidenced — September 9.** For every elected series/developer association admitted within the existing budgets, resolve the specific organisation through the shared identity registry, reuse its canonical Company UUID or create its minimal `Entity:Company` identity through a queued `identity_only` job, then replay `Game_Series → DEVELOPED_BY → Company`. No full company enrichment is required to create that identity. A display string alone is never identity or relationship evidence. Unresolved, conflicting or over-budget work retains its evidence and a pending/deferred status; it cannot be marked complete after merely adding a name to `developers`.

The series edge means **this company has evidenced development involvement in the series**, not that it developed every member. Its `development_scopes` is a deduplicated primitive array containing `series_explicit`, `member_credit`, or both. Direct support requires an exact-series developer statement, such as non-deprecated Wikidata P178 elected under the pinned source policy, or explicit official series credit. Member support requires both the exact game/company developer credit and that same game's membership in the exact series. Retain correlated evidence in `series_developer_record_ids`, with `supporting_game_ids`, `company_role_record_ids` and `membership_record_ids` for member support. These are registry/ledger IDs, not copied card names. Do not fetch unselected member games to manufacture support, expand a series claim into game-level credits, infer parent-company development, or treat a publisher/sponsor/employer as a developer.

**Finite queue branch:** the original coordinator may schedule a predeclared `series_evidence` stage for an admitted series, then freeze the combined company endpoint manifest and queue identity-only Company resolution followed by fact/graph replay. Direct series claims can yield several companies, but each reservation uses the original root and originating game's remaining limits. Shared Company identities and jobs deduplicate across series/games. Neither the series writer nor a Company leaf can admit another root, enumerate company catalogs or invoke another waterfall. [Main build rules and Counter-Strike example](/guides/archive/video-game-build-plan#evidenced-series-developers).

**Projection and correction:** `Game_Series.developers` is the sorted name projection of eligible series developer associations, not an independent source for new edges. A member-supported public association requires that member's successful publication and the public readiness of both endpoints. Mixed-support edges expose only eligible public member IDs, facts and descriptions; a direct series claim cannot reveal a separate hidden member credit. Re-election removes only withdrawn support; when the final eligible support disappears, retire the series edge and remove its projected developer name. Preserve Company identity and unrelated game/company edges. Partial source coverage does not imply deletion. Both endpoint variants retain weight 35 and `excluded_from_scoring=true`.

**Availability projection:** follow the [locked AVAILABLE_ON contract](#available-on). A release → platform edge describes that exact release. A game → platform edge is a deduplicated summary supported by explicit game availability and/or elected released scopes of the same canonical game. Both use the same owner; the summary is not a second source assertion or a second unit of scoring support. Never aggregate a separately identified remake into its original. If its final eligible scope is withdrawn, retire the summary edge. A surviving historical release supports “released on”; current store sale status is a separate time-scoped fact, so delisting does not erase release history.

**Award outcome transitions:** nomination → win keeps the award node UUID, replaces the current `Nominee` label with `Winner`, retires the current `NOMINATED_FOR` projection and activates `WON_AWARD` for the exact elected recipients. The historical nomination stays in SQL. Reverse corrections perform the converse; recipient replacement affects only that entry. Unresolved outcomes project no guessed recipient result edge. Replay/read revisions prevent mixed old/new result states from appearing as a completed projection.

**Retraction and public reads:** incomplete source coverage cannot delete omitted edges. Complete replacements retire only their source/scope, re-elect remaining evidence and deactivate an edge only when its last eligible support disappears. All game endpoint variants carry `excluded_from_scoring=true`, including reused edge names. Public traversal requires readiness of both endpoints and inherited game readiness for scoped `Game_Release` / `Game_Award` nodes. Release → platform and person → award paths remain hidden if the associated game is unpublished; checking only direct `Video_Game` endpoints is insufficient. An internal graph's existence is not public publication.

**Endpoint lock:** this build does not extend `DEVELOPED_BY` to engines/platforms, infer company ownership, mint franchise/character relationships or add expertise from generic job-title matching. Retain useful unsupported assertions for a later explicit endpoint contract. [Machine-readable baseline](/assets/video-game-waterfall/type-source-contract.json)

<a id="available-on" />

### AVAILABLE_ON

**Locked September 9, 2026 — authoritative platform-edge contract.** Go/GCP writer remains proposed. This is the only platform relationship admitted by this build; a platform's description/website creates no extra Company relationship.

| Contract | Locked behavior |
| --- | --- |
| Direction | `Video_Game → AVAILABLE_ON → Game_Platform` or `Game_Release → AVAILABLE_ON → Game_Platform` |
| Meaning | Confirmed release/availability on the exact platform. Historical release evidence does not promise current store sale or online service availability |
| Owner | `game_release_context`, consuming elected facts through the shared Go queue and graph writer |
| Identity | One semantic edge per canonical ordered pair and family; shared by all eligible evidence sources |
| Weight / scoring | `weight=80`, `layer="evidence"`, `excluded_from_scoring=true` |
| Admission | Exact platform identity **plus** elected game/release availability evidence. P400 can identify the platform but cannot alone distinguish an announced title from a released one |
| Rejection | Announcement-only, past unscoped date alone, generic “PC”, a storefront name, backward compatibility, streaming, series membership or a separately identified remake's release cannot establish this edge |

All [common game edge properties](#common-game-edge-properties-and-corrections) are required: stable `uuid`, epoch-millisecond `created_at` / `updated_at`, writing lane `source="video_game"`, attributable `source_urls` / `evidence_ids` / `chosen_fact_ids`, monotonic `projection_revision`, `policy_version`, `active`, weight/layer/scoring fields and a non-empty factual `description` with `description_generated_at`. Preserve creation time and unchanged values on no-op replay.

| Edge-specific property | Type | Requirement |
| --- | --- | --- |
| `availability_record_ids` | string[] | Required non-empty sorted unique references to elected correlated availability records for this exact game/release and platform |
| `release_entity_ids` | UUID string[] | Conditional on a game-level summary supported by separately identified releases: only those exact released scopes. Omit when support is direct game availability or the source endpoint is already the release |

Availability records retain the exact game and platform, optional release identity, source assertion/locator/revision, release-status evidence and any explicitly supported date/precision, territory, edition or channel. Unknown territory stays unknown. Do not flatten independent date/platform/territory arrays into release combinations. The only edge-specific graph fields locked in this slice are the two reference arrays above; detailed scope lives in AlloyDB.

**Creation and readiness:** the original coordinator reserves only admitted identity/metadata/fact/description/replay jobs. Reuse or queue the canonical platform identity, fulfill its [short description requirement](/guides/ontology/nodes#game-platform-properties), elect the availability record and prepare the edge description. Reuse suitable text or generate it with **GLM 5.3 FLASH, low effort**, under the [required description rule](#required-description-and-queued-creation), before edge creation. Replay through the shared parameterized writer with canonical endpoints and a current revision/lease fence. A `Game_Release` may target only the one platform in its canonical release scope; platform changes require identity/scope reconciliation rather than a second active target.

Public traversal requires an active edge, published game and ready platform. A release → platform edge requires the release's own [short description](/guides/ontology/nodes#game-release-description) and inherits that release's associated game publication; checking only the immediate endpoints is insufficient. A missing optional website does not block platform readiness. Missing release/platform description or edge description leaves the affected projection/link pending; an otherwise valid game may still publish without this optional platform link. No standalone platform card or image stage.

**Summary and correction:** direct game evidence and the same game's elected released scopes can support one game → platform edge. The summary references its supporting records; it is not another source fact. The owner re-elects support on complete scoped replacement or explicit correction, keeps other sources and retires the edge only after its last eligible support disappears. An old retry cannot reactivate it. Delisting alone does not erase historical release support. Readiness changes, source correction and description repair resume the same bounded replay; no platform catalog, Company waterfall, other game or new root is launched.

**Example:** Elden Ring → `AVAILABLE_ON` → PlayStation 4. Its exact platform statement is `Q64826862$d6f61228-4f0b-acd9-819b-f736b718d850` (P400 → `Q5014725`), with release support from [Bandai Namco's official game page](https://en.bandainamcoent.eu/elden-ring/elden-ring). A suitable short edge description is “Elden Ring was released for PlayStation 4.” The [corrected simulation](/assets/video-game-waterfall/graph-simulation.json) retains the matching platform/availability record; the source's series-membership statement is not availability evidence. Election, IDs and processing state in the example remain simulated.

<a id="uses-engine" />

### USES_ENGINE

**Locked September 9, 2026 — authoritative engine-edge contract.** This entry and the [Game_Engine node contract](/guides/ontology/nodes#game-engine-properties) are the source of truth for the Go/GCP build. Writers remain to be implemented.

| Contract | Locked behavior |
| --- | --- |
| Direction | `Video_Game → USES_ENGINE → Game_Engine` |
| Meaning | Explicit use of the engine by this exact game, with any evidenced version/build/platform/time scope retained |
| Owner | `game_context` fact/graph replay in the shared Go application |
| Identity | One semantic edge per canonical ordered game/engine-family pair, shared by all supporting sources and versions |
| Weight / scoring | `weight=80`, `layer="evidence"`, `excluded_from_scoring=true` |
| Admission | Elected exact-game engine-use evidence from typed source claims, an official development report or an enabled permitted catalog. Resolve both endpoint identities before writing |
| Rejection | Engine-maker product pages, studio tooling, employment, engine-rendered trailers, SDK/audio/physics middleware use and another game's engine cannot independently establish this relationship |

All [common game edge properties](#common-game-edge-properties-and-corrections) are required: stable `uuid`; epoch-millisecond `created_at` / `updated_at`; `source="video_game"`; non-empty attributable `source_urls`, `evidence_ids`, `chosen_fact_ids`; monotonic `projection_revision`; `policy_version`, `active`; the fixed weight/layer/scoring fields; and non-empty factual `description` with `description_generated_at`. Preserve creation time and no-op values across replay. `source` names the writing lane; the ledger retains the actual provider, such as `official_xbox`.

| Edge-specific property | Type | Locked requirement |
| --- | --- | --- |
| `engine_use_record_ids` | string[] | Required, non-empty sorted unique references to elected correlated engine-use records for this exact game and engine family |
| `engine_versions` | string[] | Optional sorted unique literal version values from surviving eligible use records, e.g. `["5"]`. Omit when unknown; never invent `5.0`, a minor version or the latest version |

Each use record binds the canonical game and engine to its source evidence, version literal when present, and only explicitly supported build/release/platform/time qualifications. Preserve unknown scope as unknown. The version array is a summary of those records, not evidence that every version applies to every platform or build. Engine-family metadata and descriptions belong to `Game_Engine`; version scope does not create extra engine nodes. Multiple evidenced engine families may coexist for a game with their own scope; do not impose an unsupported single-engine rule.

**Queued creation:** the original coordinator reserves the exact engine identity and predeclared later fact, description and replay work within the existing root manifest/budgets. Reuse the canonical engine or queue its bounded `identity_only` resolution. Elect scoped use records; prepare the short node description and the separate edge description. Reuse suitable supported text or use the shared **GLM 5.3 FLASH, low-effort** description jobs. The edge needs validated non-empty text before first materialization. `game_context` then uses the shared parameterized writer with canonical IDs, revision checks and a current lease fence. Evidence/provider text is data, never executable syntax. No source lookup or full engine/company/game waterfall starts from a graph write or leaf job.

**Readiness and corrections:** public traversal requires an active relation, a published game and a ready engine with its short description. Missing identity, evidence or description leaves only this optional engine/link pending; an otherwise valid game can publish. All attempts and failures use the [shared Universe movie/music logging](/guides/open-work/video-game-waterfall#shared-universe-error-logging-required). Complete same-scope replacements or explicit corrections re-elect surviving use records, recompute version summaries and refresh invalidated text. Partial omission retracts nothing. A version correction preserves the same edge UUID; changing a game's engine preserves independently supported historical scope. Retire an edge only when its final eligible support is withdrawn. Stale retries cannot restore withdrawn facts, and retraction does not wait for description generation. Preserve engine nodes and other games' links.

**Locked real-data example:** Senua's Saga: Hellblade II → `USES_ENGINE` → Unreal Engine, with `engine_versions=["5"]`. [Xbox's development report](https://news.xbox.com/en-us/2024/05/16/hellblade-2-performance-capture-next-level-combat-design/) describes the game's combat development in Unreal Engine 5. This supports the described use; it does not certify every future build or port. The example description remains “Senua's Saga: Hellblade II uses Unreal Engine.”

The [complete edge JSON and correlated source record](/assets/video-game-waterfall/support-type-examples.json) now include `source` and `evidence_ids`. Example IDs, fact election, timestamps, revisions and description output are simulated; the evidence reference is an illustrative ledger key, not an identifier issued by Xbox. No live graph write or model call was performed. [Rendered node and edge example](/guides/open-work/video-game-waterfall-sources#real-engine-and-release-examples).

<a id="game-has-award" />

### HAS_AWARD — video game

**Locked September 9, 2026 — authoritative game recognition edge.** This endpoint extension pairs with the [Game_Award node contract](/guides/ontology/nodes#game-award-properties). The existing movie/theatre contracts retain their own scope; this lock is for `Video_Game → Game_Award`. Go implementation remains pending.

| Contract | Locked behavior |
| --- | --- |
| Direction / meaning | `Video_Game → HAS_AWARD → Game_Award`: this exact game is associated with this resolved nominee entry; named-person entries remain recognition context for their associated game |
| Owner | `game_recognition` fact/graph replay in the shared Go application |
| Identity | One semantic edge per canonical ordered game/award-entry pair; another source or a changed outcome does not create a second edge |
| Weight / scoring | `weight=40`, `layer="evidence"`, `excluded_from_scoring=true` |
| Admission | Elected exact-game + authority/ceremony/category/nominee-entry evidence; resolve endpoint UUIDs and preserve named/joint recipient scope |
| `award_evidence_record_ids` | Required non-empty string list referencing correlated SQL records containing the game, award entry, ceremony/category, outcome, source statement/qualifiers and elected fact |
| `award_result` | Current elected `nominee` or `winner`, matching the target node's `result` and modifier in the same public recognition revision; never an identity field |
| Required descriptions | Non-empty supported edge description before first materialization; describe a person's nomination/win as that entry's game context, not a win by every contributor |
| Rejection | Page proximity, presenter/guest/credit-list membership, company billing or an unresolved outcome cannot establish a nomination/win. A category alone cannot prove a recipient, adaptation or game release |
| Publication | Both endpoints ready, award's own valid description/elected outcome, associated game published with valid media, and node/edge outcome projections consistent. `active=true` does not override these checks |

All [common game edge properties](#common-game-edge-properties-and-corrections) are required: stable `uuid`; epoch-millisecond `created_at` / `updated_at`; writing lane `source="video_game"`; non-empty `source_urls`, `evidence_ids`, `chosen_fact_ids`; `projection_revision`, `policy_version`, `active`; weight/layer/scoring fields; `description` and `description_generated_at`. Actual provider provenance remains in the evidence ledger. The full approved envelope and two award-specific properties are shown below.

**Creation order:** bounded `award_evidence` → exact entry/endpoint resolution and fact election → queued node description and edge `describe` work when suitable text is missing → `game_recognition` replay → matching public publication. Edge descriptions use the shared [GLM 5.3 FLASH, low-effort rule](#required-description-and-queued-creation). Every stage consumes the original root budgets and shared movie/music logging; source workers, identity-only endpoints and replay writers cannot launch discovery, new award years or another game/person/company waterfall. An optional pending award/link does not block an otherwise publishable parent game.

**Corrections and retirement:** preserve node/edge UUIDs and creation times. Re-elect the exact scoped records and refresh affected name/description, current outcome labels, `award_result` and separately evidenced recipient edges. Apply one fenced recognition revision, or keep the new revision hidden until every affected projection agrees. Preserve nomination history in SQL. Unsupported old winner text cannot stay public while generation is pending; retractions never wait for a model. Missing entries in partial or winners-only pages cannot retract nominees. Complete same-source/scope replacements retire only their own support; retire the edge only after its final eligible support disappears. Stale retries cannot restore withdrawn outcomes.

**Locked example:** Elden Ring's game-only Best Game Direction win. The referenced record preserves the frozen P166 statement and P585/P805 qualifiers for the exact category and 2022 ceremony. Internal IDs, timestamps, election/replay state and description output are simulated; no production edge or LLM generation receipt is claimed.
...
The [complete example and its source records](/guides/archive/video-game-build-plan#complete-award-node-and-game-edge) remain synchronized with the JSON fixture. No individual recipient edge is inferred for this game-only entry. Award artwork is a resolved game media reference, with no new artwork edge or upload.

### Shared names and replay ownership

**`PUBLISHED_BY`:** publication → company keeps its existing weight **20**; the implemented game → company variant is **45**. The registry, description dispatch and scoring reader must consider endpoint families. One source can support two regional publishing scopes without creating two uncorrelated territory/platform arrays. Elden Ring → FromSoftware has a Japan publishing scope; Elden Ring → Bandai Namco has a separate worldwide source assertion. Neither source scope is silently rewritten.

**`RELEASE_OF`:** both game and music variants use 30, but a `Game_Release` is not a music `Release`. The owner resolves the appropriate release identity and writes only its allowed endpoint pair. Game snapshot jobs cannot invoke an artist/album catalog waterfall.

<a id="game-based-on" />

**`BASED_ON`:** this game build emits derivative → source with at least one `Video_Game` endpoint; the other endpoint may be an independently resolved `Film_TV`, `Work:Theatrical_Work`, `Stage_Production` (only when that exact production is evidenced) or `Video_Game`. P144 already points derivative → source; P4969 is inverted once. A source may have only one of those statements. An inverse lookup remains one bounded hop. The shared adaptation replay owner emits one semantic edge and preserves both statements if present.

The older [theatre `ADAPTED_FROM` proposal](#stage-adapted-from) was superseded by `BASED_ON` on September 7. The shared Go writer supports the allowed game, film and theatre endpoints; do not emit both names for the same adaptation. A production's already resolved underlying work can supply intake context without moving the adaptation edge or copying the production's cast to the game.

**Awards and music:** game award endpoints extend the existing award families at 40/30/35, with named recipients required. Music owners retain their own internal relationships; the bridge writer adds only the exact elected soundtrack/use relation. All owners use the shared [Universe error-logging contract](/guides/open-work/video-game-waterfall#shared-universe-error-logging-required).

### Common game edge properties and corrections

| Property group | Required meaning |
| --- | --- |
| `uuid`, `created_at`, `updated_at` | Stable semantic edge identity; preserve creation time; application-managed update time |
| `source`, `source_urls`, `evidence_ids`, `chosen_fact_ids` | Owning source lane plus attributable elected evidence; source assertions alone cannot create an edge |
| `projection_revision`, `policy_version`, `active` | Versioned, generation-fenced application/retraction; stale retries cannot resurrect retired evidence |
| `weight`, `layer`, `excluded_from_scoring` | Positive endpoint-specific weight, declared layer and mandatory pilot scoring exclusion |
| `description`, `description_generated_at` | Required non-empty, evidence-grounded description before creating an edge; missing suitable text uses queued GLM 5.3 FLASH at low effort under the [description rule](#required-description-and-queued-creation). BASED_ON retains its 25–45-word sentence convention |
| Scoped relation/credit record references | Preserve correlated role/character/language/platform/territory/date facts in SQL; graph properties use primitive scalars/lists |

One `WORKED_ON` edge can retain multiple roles: Miyazaki is both director and writer in the Elden Ring source snapshot. A partial or failed source fetch cannot retract omitted credits. A successfully committed complete replacement or explicit correction can retire its own evidence; re-election retains other source support. Pending endpoints trigger replay only, never discovery. Images are properties/media references and create no image edge.

### Real-data edge breakdown and examples

The [downloadable simulation](/assets/video-game-waterfall/graph-simulation.json) contains **30 edges across 10 relationship names**. Its counts include already-owned movie/music/theatre context, not just newly proposed game edges.

| Edge | Count | Real example in the fixture |
| --- | ---: | --- |
| `WORKED_ON` | 16 | 14 game credits plus 2 existing Last of Us TV credits; Miyazaki's two roles occupy one edge |
| `DEVELOPED_BY` | 2 | Elden Ring → FromSoftware; The Last of Us → Naughty Dog |
| `PUBLISHED_BY` | 3 | Two scoped Elden Ring publishers and one Last of Us publisher |
| `BASED_ON` | 2 | The Last of Us TV → 2013 game; Hamlet game → Shakespeare's play |
| `SOUNDTRACK_FOR` | 1 | The Last of Us soundtrack Release_Group → game |
| `RELEASED` | 1 | Gustavo Santaolalla → soundtrack album; existing music ownership |
| `IN_GAME_SERIES` | 1 | Elden Ring → Elden Ring series |
| `AVAILABLE_ON` | 2 | Elden Ring → Windows and PlayStation 4 |
| `HAS_AWARD` | 1 | Elden Ring → scoped 2022 Best Game Direction result |
| `WROTE` | 1 | Shakespeare → Hamlet play; existing theatre ownership |

Other manifest rows are later contracts with no claimed instance in this fixture. Inspect [full edge property examples](/guides/open-work/video-game-waterfall-examples), [frozen source statements](/assets/video-game-waterfall/source-evidence.json) and [the proposed game weights](/guides/ontology/edge-weights#video-game-weights). Neither graph traversal nor an existing context edge authorizes another waterfall run.
...
These writers create the structural graph edges. Description generation is handled separately by the generic music edge description trio. (`mb/tool/backfill-genre-area-edges` #13103 is an excluded backfill sweep, not part of the forward-path writer set — genres are properties, and `FROM_AREA` was a forward gap in this legacy Xano extract; the Go writer now exists and was observed in the September 22 audit.)
...
| `FROM_AREA` | `(Artist / Company:Music_Label / Place)->(Country / Region / City)` | **70** | `area_type` | Go writer implemented; observed. Example: "Nirvana is from the United States." |
...
<Info>
**Status:** Spec locked 2026-09-05 (proposal reviewed 2026-07-09; all open questions resolved — see [Design decisions](#design-decisions)) — companion to the [`Event:Live_*` node model](/guides/ontology/nodes#live_event). The node itself shipped on 2026-04-18; most of these edges remain proposed. **`SPOKE_AT` now has a Go Deep Bio writer and observed edges** (September 22 audit). The legacy App exception is: **`REGISTERED_FOR`** is live — `(Person)-[:REGISTERED_FOR]->(Live_Event)` at weight **65**, written by the LSI registration path (`temp/add-lsi-edges` #12864). Indexes for `SPONSORS` and `ATTENDED_EVENT` were materialized on 2026-07-06, but the full writers are pending.
</Info>
...
<Info>
**Implementation checked September 22, 2026.** `internal/platform/graph/person_relations.go` implements the Deep Bio person relationships below. The dev graph contains `COFOUNDED_WITH`, `COLLEAGUES_AT`, `REPORTED_TO`, `COLLABORATORS`, `CO_INVESTORS`, `SUCCEEDED_BY`, `MARRIED_TO` and `MENTOR`; `PARENT_OF` is registered/implemented but was not observed. The original September 5 ledger batch and proposed weights are historical provenance, not current deployment counts. Remaining family/ledger proposals stay separately marked.
</Info>
...
| --- | --- | --- | --- |
| `co_founder` | COFOUNDED_WITH | symmetric | specified below |
| `colleague`, `board_peer` | COLLEAGUES_AT | symmetric | specified below |
| `manager` / `direct_report` | REPORTED_TO | subject → manager / report → subject | specified below |
| `collaborator` | COLLABORATORS | symmetric | specified below |
| `co_investor` | CO_INVESTORS | symmetric | specified below |
| `predecessor` / `successor` | SUCCEEDED_BY | predecessor → subject / subject → successor | specified below |
| `spouse` | MARRIED_TO | symmetric | specified below |
| `parent` / `child` | PARENT_OF | parent → subject / subject → child | specified below |
...
| works `podcast` | APPEARED_IN (15) | subject → `MediaAppearance` | adopts the v1.2 media lane's [APPEARED_IN](#appeared_in) (decided 2026-09-05); the ledger's interim `APPEARED_ON` name is retired |
...
<a id="enrichment-engine-v12-edges" />

## Enrichment Engine v1.2 Edges

<Info>
**Status updated September 22, 2026:** partially implemented. Stage credits/awards, Deep Bio appearances, IRS governance and donor graph writers exist; expanded context/media/filing contracts remain proposed where noted. The original contracts came from the v1.2 build plans (August 21/25). Folded onto this page 2026-09-05 so the ontology has one inventory; the lane pages remain the build specs — [Broadway co-producers](/guides/open-work/roadmap/broadway-co-producers), [Media co-occurrence](/guides/open-work/orbiter-univers-standalone/social-media-enrichment), [990 / FEC / Form D](/guides/open-work/roadmap/990-fec-form-d), [Donors](/guides/open-work/roadmap/donors). <span style={{ color: "#16a34a", fontWeight: 600 }}>Green</span> names follow the same convention as the section above: in the spec, no live writer.
</Info>
...
The proposed weighted v1.2 contracts use the 2026-07-04 envelope (`uuid`, `created_at`, `updated_at`, `weight`, `description`, `description_generated_at`) **plus `source`** — the writing lane or provider (`irs_990`, `deep_bio`, `media.youtube`, `fundable`, …; the live person-edge writer already stamps it on every `WORKS_AT` / `BOARD_MEMBER_OF` / `FOUNDED`). `source` says *who wrote this*; the per-edge `source_urls` say *where the evidence is* — public-source variants require both and they are not interchangeable. The theatre submission-only pilot variant may omit public URLs, while retaining mandatory source names and immutable evidence/provenance references under its [specific contract](#theatre-edges). QA queries scope a lane's edges by `r.source`. (Added to the envelope 2026-09-06, Mark.) `description` is the deterministic sentence given per edge. Property tables below list only the edge-specific fields.

<a id="theatre-edges" />

<a id="theatre-relationships--proposed-go-contract" />

### Theatre relationships — Go implementation and remaining proposals

<Info>
  **Implementation checked September 22, 2026:** `PRODUCED`, stage `WORKED_ON`, `BASED_ON` and stage award writers exist. `PRODUCTION_OF`, `STAGED_AT` and theatrical-work authorship remain proposals; the award recipient writer currently supports Person only. No theatre edges were observed in the dev snapshot. This replaces the earlier stage-as-`Film_TV` proposal. Endpoints follow [Node Types → Theatre](/guides/ontology/nodes#theatre-nodes); delivery follows the [Go build plan](/guides/open-work/roadmap/broadway-co-producers). The property examples below are synthetic contract examples, not records written to any graph.
</Info>

The [closed build output manifest](/guides/open-work/roadmap/broadway-co-producers) is the developer checklist; every relationship below has complete property data, with reused endpoint ownership documented in Node Types.

**Three new relationship names are proposed:** `PRODUCTION_OF`, `STAGED_AT` and `BASED_ON` (which replaces the earlier `ADAPTED_FROM` proposal, 2026-09-07). Existing names cover producing, creative authorship and awards. Reusing a name does not make its current movie/music writer valid for new endpoint types.
...
| **`BASED_ON`** — new, replaces `ADAPTED_FROM` | `derivative work → source work` across `Film_TV`, `Stage_Production` and `Work:Theatrical_Work` | Proposed **55** | Evidence context, excluded; adaptation slice after 3b (Mark 2026-09-07) |
...
**New, adaptation slice after Broadway 3b (Mark, 2026-09-07). Replaces the `ADAPTED_FROM` proposal below, which stays for history.** **Pattern:** `derivative work → BASED_ON → source work`, one direction only: the edge runs from the thing that was made later to the thing it was made from. Wikidata's "based on" (P144) is the same fact read from the derivative side and "derivative work" (P4969) the same fact read from the source side; the graph keeps one edge and reads it either way. **Proposed weight: 55; evidence layer, excluded from scoring.** Owner: an adaptation lane that reads Wikidata for the fact and the film gate for the film endpoint.

Endpoints: any pair among `Entity:Film_TV`, `Entity:Stage_Production` and `Entity:Work:Theatrical_Work`, each independently resolved through its own gate. A film reaches the graph through its IMDb id, which the Wikidata film item carries (P345); a stage work through the theatre lane. A source that has no supported identity (a novel, a short story) stays SQL evidence until a supported type exists; never invent a node for it and never use `SAME_AS`. A chain is separate edges: the 2021 film has its own edge to the 1961 film and its own edge to the 1957 musical, each described on its own.

**What decides the edge and what writes the sentence are different things.** Wikidata decides whether the edge exists, its direction and the IMDb id it points at. A grounded model call writes only the `description`, from the two endpoint nodes and the Wikidata qualifiers plus web search for fidelity, and returns `SKIP` when the input is not a derivative-work relation; it never decides the edge. The description characterizes the relationship, not the endpoints: both nodes already carry their own descriptions, so restating creators, venues or awards on the edge is duplication that goes stale twice. It answers what kind of derivation, how faithful, what changed, who carried over, and the time gap, in one sentence of 25 to 45 words that reads correctly when a system sees only the edge and the two node labels.

| Property | Type / requirement | Contract |
| --- | --- | --- |
| `derivation_type` | enum · required | `adaptation`, `remake`, `sequel`, `retelling`, `translation`; the query-side twin of the sentence. The model emits both as JSON |
| `fidelity` | enum · optional | `faithful`, `loose`, `substantially_reworked`; empty when search could not establish it |
| `source_property` | string · required | `wikidata:P144` today; another authority names itself |
| `source_qid` / `target_qid` | string · required | The Wikidata items the fact was read from |
| `chosen_fact_id` | UUID string · required | The elected relation fact, one per (derivative, source) pair |
| `years` | object · optional | `{ "source": 1957, "derivative": 1961 }` as Wikidata states them; the edge sentence names both |
| `description` | string · required | The one-sentence relationship description below; `description_generated_at` beside it |

**Description contract** (system prompt for the writer, verbatim):

```text
You write the "description" property for a derivative-work edge in a knowledge graph. The edge runs from a target work to the source work it is based on (Wikidata P144). Input is JSON with both endpoint nodes (labels, types, dates, creators) and any Wikidata qualifiers on the relationship.

Write one sentence, 25 to 45 words, describing the relationship itself. It must read correctly on its own when a system sees only this edge and the two node labels.

Cover, in this priority: (1) the kind of derivation (adaptation, remake, sequel, loose retelling, translation, stage-to-screen, etc.) with both years; (2) fidelity to the source and the principal changes (medium, setting, structure, material cut or added); (3) continuity of creators across the two works, if any; (4) nothing else.

Rules:
- Do not restate facts that belong to a node (full creator lists, premiere venue, awards, cast). Name a person only when their involvement in both works is the point.
- Use only the input or facts verified by web search. Use search to establish fidelity and changes, which Wikidata rarely encodes. Never invent. If fidelity cannot be established, describe the derivation type and years only.
- Wikidata values win over web results unless the web source is authoritative and the Wikidata value is clearly wrong.
- Plain text, present tense, no subject restated ("Faithful 1961 screen adaptation of..." not "The 1961 film is a faithful..."). No markdown, no citations, no em-dashes, no evaluative adjectives beyond fidelity terms (faithful, loose, substantially reworked).
- If the target is based on multiple sources, describe only the edge to the source given in the input.
- If the input is not a derivative-work edge, return the single word: SKIP.

Output only the sentence.
```

The model emits `{ "derivation_type": ..., "fidelity": ..., "description": ... }`; the sentence is for humans and LLM context, the enums are for queries. Run the writer once per edge, never once per chain, so each edge in `2021 film → 1961 film → 1957 musical` gets its own scoped sentence.

**Worked example** — `West Side Story (1961 film) → BASED_ON → West Side Story (1957 musical)`:

```json
{
  "edge_type": "BASED_ON",
  "from": {
    "node_uuid": "00000000-0000-4000-8000-000000000021",
    "entity_id": "00000000-0000-4000-8000-000000000021",
    "node_family": "Film_TV",
    "label": "West Side Story (1961)"
  },
  "to": {
    "node_uuid": "00000000-0000-4000-8000-000000000022",
    "entity_id": "00000000-0000-4000-8000-000000000022",
    "node_family": "Stage_Production",
    "label": "West Side Story (Broadway, Winter Garden Theatre, 1957)"
  },
  "properties": {
    ...
    "target_qid": "Q1064717",
    "source_urls": [
      "https://www.wikidata.org/wiki/Q1064710"
    ],
    "chosen_fact_id": "00000000-0000-4000-8000-000000000502",
    "projection_revision": 1,
    "policy_version": "adaptations-v1",
    "active": true,
    "weight": 55,
    "layer": "evidence",
    "excluded_from_scoring": true,
    ...
    "fidelity": "faithful",
    "years": { "source": 1957, "derivative": 1961 },
    "description": "Faithful 1961 screen adaptation of the 1957 stage musical, co-directed by original choreographer Jerome Robbins with Robert Wise; retains the score and book, reorders some songs, and moves the action to real New York locations."
  }
}
```

The 2021 film gets two edges of its own: one to the 1961 film (`remake`, and the sentence says it returned to the stage text rather than remaking the earlier film) and one to the 1957 musical (`adaptation`). Neither edge inherits the other's sentence.

### <span style={{ color: "#16a34a", fontWeight: 600 }}>ADAPTED_FROM</span>

**Superseded by [`BASED_ON`](#based-on) on 2026-09-07; kept for the record of the earlier proposal.** **Was: New, later optional slice. Pattern:** `Entity:Work:Theatrical_Work → ADAPTED_FROM → resolved source work`. **Proposed weight: 55; evidence layer, excluded from scoring.** Owner: facts-elected work-lineage claim and stage work replay.
...
**Game endpoint boundary (September 8, 2026):** this older theatre proposal does not independently claim game relationships. If either endpoint is `Video_Game`, use the shared [game `BASED_ON` contract](#game-based-on), with one elected relation and one replay owner. Do not emit both names for the same adaptation.
...
**Go proposal revised 2026-09-06; no writer is implemented by this documentation pass.** [Media build plan](/guides/open-work/orbiter-univers-standalone/social-media-enrichment) · [Node contracts](/guides/ontology/nodes#media-appearances). Keep the 15/20/45 appearance and 55 news bands. **Accepted by Mark, 2026-09-18:** use `SESSION_OF` at 30 to resolve the old media `PART_OF` collision with the settled project reservation. All source/graph/scoring flags start disabled.
...
| **Writer** | orbiter-universe Deep Bio appearance replay (`person_appearance_replay.go`, 2026-09-07) for `works[].type = podcast` rows → `media_appearance_participants` → this edge, `MediaAppearance` MERGEd on `media_key = podcast::<episode url>`; the [Media co-occurrence lane](/guides/open-work/orbiter-univers-standalone/social-media-enrichment) will share the node |
...
**Accepted name replacing the unbuilt media `PART_OF` proposal (Mark, 2026-09-18).** The old anchor is retained for existing documentation links. User-graph project `PART_OF` remains unchanged; `SUB_EVENT_OF` still connects one live event to another. The edge remains Planned until its writer lands.
...
Quick scan of all edge weights, sorted strongest → weakest. One integer scale (1–100, lower = stronger) — the film/TV edges' old sub-1 values were raised by the §3.6 audit. Green rows are remaining specifications; use the [current audit](#current-universe-writers-and-parameters) and generated CSV for endpoint-specific writer status.

| Edge | Weight |
|------|-------:|
| `SAME_AS` (music label→company · music artist→person) | **1** |
| COFOUNDED_WITH (Person ↔ Person) | **1** |
| PARENT_OF (Person → Person) | **2** |
| MARRIED_TO (Person ↔ Person) | **4** |
| `FOUNDED` | **5** |
...
| `MENTOR` | **14** |
| OFFICER_OF (Person → nonprofit Company) | **14** officer · **20** key employee |
| `AUTHORED` | 15-25 (age-tiered, older = higher; corrected 2026-09-22) |
| `CONTRIBUTED_TO` | 15-40 (age-tiered, older = higher; corrected 2026-09-22) |
...
| REPORTED_TO (Person report → Person manager) | **15** current · **25** past |
| COLLABORATORS (Person ↔ Person) | **15** |
| <span style={{ color: "#16a34a", fontWeight: 600 }}>RELATED_PERSON_OF</span> (Person → issuer Company) | **15** officer / director · **20** promoter |
| APPEARED_IN (Person → `MediaAppearance`) | **15** interview · **20** panelist/moderator · **45** speaker (15 below a verified complete hub threshold; pair-aware policy required) |
| COLLEAGUES_AT (Person ↔ Person) | **16 / 24** board peers · **20 / 28** colleagues (current / past) |
...
| `PUBLISHED_BY` | **20** |
| `MEMBER_OF` | **20** |
| CO_INVESTORS (Person ↔ Person) | **20** same round · **25** default |
...
| SUCCEEDED_BY (Person predecessor → Person successor) | **30** |
| DONOR_TO (Person\|Company → institution) | **30 / 33 / 36** person by amount · **30–40** foundation · **35** corporate · **38 / 48** derived · **50** reserved |
| `HEADLINED` | **35** |
| `RECORDED_AT` | **35** |
...
**Theatre endpoint additions (audited September 22, 2026):** partial implementation; see the [current writer differences](#corrected-implementation-differences). Only `PRODUCED` participates in the first-release co-credit scoring policy. These rows are also tracked in [Edge Weights → Theatre](/guides/ontology/edge-weights#theatre-weights).
...
| `BASED_ON` → resolved source work | **55** | Writer implemented; excluded, evidence context |
