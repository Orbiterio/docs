...
<Note>
**Design only.** The prototype uses illustrative contacts and local interactions. It does not call AI, send email, store real consent, or connect to the application. Its toolbar switches between Setup, Recipient page, Emails, and Tracking. Edits survive view changes within the preview and reset on refresh. Screenshots below show the product views without the preview toolbar. The [Warm Intro Build Plan](/guides/open-work/warm-intro-feature/warm-intro-build-plan) now documents the proposed tables, history, APIs, parallel drafting, prompts, email templates, and implementation order.
</Note>
...
**Selected direction: Three composers.** The setup starts from Direction 1: horizontal stages above three compact email composers, with the entire communication sequence visible together.

<Frame caption="Selected Direction 1: three mini email composers, aligned beneath horizontal stages, using the current Orbiter Midnight styling.">
  <img src="/images/warm-intro/01-setup.png" alt="Full-screen Warm Intro setup for Avery and Sarah. Compact person cards and a switch-order control sit above an expandable WHY strip. Three horizontal stages align with side-by-side composers for the two requests and the final introduction. Each message has its own review action, followed by one start action for the sequence." />
</Frame>
...
- The **Switch who is asked first** control between the person cards swaps the sequence before launch. The recommendation remains distinguishable from a manual choice.
...
| Title | Both people’s names beneath **Double opt-in introduction**, followed by compact person cards. |
| WHY strip | Full-width suggestion headline in the current outlined Midnight blue treatment. The disclosure expands the full private context. |
| Horizontal stages | **1 Ask Avery**, **2 Ask Sarah**, **3 Introduce them** align with the three composer columns. Each names its send condition and shows a review check when reviewed. Selecting a stage focuses its composer; all three remain visible. |
| Order control | A swap control between the person cards changes who is asked first. The adjacent relationship note distinguishes the suggested order from a manual choice. |
| Three composers | Compact mail windows each contain **To**, **Subject**, editable message, an email-preview toggle, and **AI drafted**. Requests also offer **Preview public page**. The final composer shows both recipients and the sender copied. |
| Composer footer | Each message has its own **Mark reviewed** action and send condition. Review messages in any order, without advancing or hiding drafts. Review is distinct from recipient acceptance. |
| Start area | One full-width footer contains the review count, a plain-language sequence summary, and **Make the warm intro**. Nothing sends until this action; later messages wait for consent. |

All three drafts stay on screen as familiar mini email composers. This carries forward the useful side-by-side detail from the early designs while using Orbiter’s current surfaces, compact controls, and petrol action treatment. The WHY is available above the messages without taking a permanent side column.
...
| Setup stages | Blue focus underline and numbered circles; muted green checks for reviewed drafts. Triggers stay visible below each stage title. |
| Action area and button | `#14323b` area, `#1a424b` button, `#3e7380` border, `#b0dce3` text |
| Setup composers | `#0b1018` editor, `#121b2a` mail header, `#35445d` border, 8 px radius. To and Subject use familiar mail dividers. |
...
The full-screen workspace uses three equal composer columns, with matching horizontal stage columns above. The participant strip and expandable WHY span the workspace. At 650 px and below, composers stack in sequence while the three stage controls remain horizontal; selecting one moves focus to that composer. Message areas grow to show the full draft instead of creating internal scrolling. The single start action follows the three composers on small screens.
...
- Selecting a setup stage moves focus to its composer without approving it or hiding the other drafts. The review and launch controls have explicit action labels.
...
This page covers the experience, copy, state model, and visual reference. Backend orchestration, consent-link mechanics, Resend integration, storage, and delivery recovery implementation are addressed in the [Warm Intro Build Plan](/guides/open-work/warm-intro-feature/warm-intro-build-plan).
