...
**Detailed build-plan proposal · September 25, 2026.** The [Warm Intro Design](/guides/open-work/warm-intro-feature/warm-intro-design) is the working design baseline. Build the selected **Direction 1: three mini email composers beneath horizontal stages**, the mobile invitation, and the tracking modal. This page plans the application implementation; the existing HTML remains a design prototype.
...
The WHY is first-class text, not merely a score, tag, event label, or blob inside the suggestion. Preserve three distinct things:

| Field | Meaning | History rule |
| --- | --- | --- |
| `source_why_text` | The original natural-language description from the suggestion, including all paragraphs | Capture verbatim; preserve the original source JSON alongside it. Never overwrite it with an AI summary. |
| `why_connect` | The working explanation of why these two people should be introduced | Copy the original when appropriate; every saved edit appends a context version. |
...
| `public_why` | An optional sender-approved explanation safe to disclose | Separate from private intent and internal context; recipient copy still requires message review. |

If the existing paragraph blends the connection rationale and sender intent, keep the whole original and use it as the working rationale; leave the separately structured intent null unless the source or user explicitly provides it. Preserve provenance. If neither is available, record `why_status=not_provided`, offer an optional explanation in setup, and allow a neutral draft without fabricated rationale. An absent WHY is not an excuse to lose the original intro record.
...
| `state` | `text` · required | Go-validated lifecycle vocabulary. |
...
| `current_stage` | `text` · required | draft, first_request, second_request, final_introduction, or closed. |
...
**Keys and indexes:** `UNIQUE (id, owner_user_id)`
...
**Keys and indexes:** `UNIQUE (introduction_id, id)`; `UNIQUE (introduction_id, original_person_id)`; `UNIQUE (introduction_id, source_role)`
...
| `source_why_text` | `text` · nullable | Original natural-language WHY, verbatim with paragraph breaks; never replace with a label/score. |
| `why_connect` | `text` · nullable | Working natural-language explanation of why the two people should connect. |
...
| `public_why` | `text` · nullable | Sender-reviewed common explanation allowed in recipient-facing copy, when supplied; do not auto-publish private WHY. |
| `sender_snapshot` | `jsonb` · required | Display name, verified reply address and signature policy at this version. |
...
| `change_reason` | `text` · required | source_loaded, why_edited, address_changed, order_changed, profile_changed, or sender_changed. |
| `created_by_user_id` | `uuid` · nullable | Editor identity; nullable for system capture. |
| `created_at` | `timestamptz` · required | Append time. |

**Keys and indexes:** `UNIQUE (introduction_id, id)`; `UNIQUE (introduction_id, version)`; `FOREIGN KEY (introduction_id, first_participant_id) REFERENCES warm_intros.participants(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, second_participant_id) REFERENCES warm_intros.participants(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, recommended_first_participant_id) REFERENCES warm_intros.participants(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `UNIQUE (introduction_id, id)`; `UNIQUE NULLS NOT DISTINCT (introduction_id, kind, participant_id)`; `FOREIGN KEY (introduction_id, participant_id) REFERENCES warm_intros.participants(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `UNIQUE (introduction_id, id)`; `UNIQUE (message_id, id)`; `UNIQUE (message_id, version)`; `FOREIGN KEY (introduction_id, message_id) REFERENCES warm_intros.messages(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, context_version_id) REFERENCES warm_intros.context_versions(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `UNIQUE (message_id, message_version_id, context_version_id)`; `FOREIGN KEY (introduction_id, message_id) REFERENCES warm_intros.messages(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (message_id, message_version_id) REFERENCES warm_intros.message_versions(message_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, context_version_id) REFERENCES warm_intros.context_versions(introduction_id, id) ON DELETE RESTRICT`
...
| `token_version` | `integer` · required | Bind requests to this capability version. |
...
**Keys and indexes:** `UNIQUE (introduction_id, id)`; `UNIQUE (introduction_id, participant_id)`; `FOREIGN KEY (introduction_id, participant_id) REFERENCES warm_intros.participants(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, context_version_id) REFERENCES warm_intros.context_versions(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, message_id) REFERENCES warm_intros.messages(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `UNIQUE (introduction_id, id)`; `FOREIGN KEY (introduction_id, invitation_id) REFERENCES warm_intros.invitations(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, context_version_id) REFERENCES warm_intros.context_versions(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `UNIQUE (decision_id, revision)`; `FOREIGN KEY (introduction_id, decision_id) REFERENCES warm_intros.consent_decisions(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `UNIQUE (introduction_id, id)`; `UNIQUE (message_id, id)`; `FOREIGN KEY (introduction_id, message_id) REFERENCES warm_intros.messages(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, context_version_id) REFERENCES warm_intros.context_versions(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `PRIMARY KEY (introduction_id, seq)`; `FOREIGN KEY (introduction_id, run_id) REFERENCES warm_intros.generation_runs(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, message_id) REFERENCES warm_intros.messages(introduction_id, id) ON DELETE RESTRICT`
...
| `payload_ciphertext` | `bytea` · nullable | Exact provider request, including capability URL; required before dispatch, encrypted at rest. |
| `encryption_key_version` | `text` · nullable | Key version for decrypting exact payload. |
| `payload_sha256` | `bytea` · nullable | Hash of complete canonical provider request; stable on retry. |
...
| `delivery_facts` | `jsonb` · required | Per-envelope-recipient delivered/bounce/complaint evidence; preserve distinct facts. |
...
**Keys and indexes:** `UNIQUE (introduction_id, id)`; `FOREIGN KEY (introduction_id, message_id) REFERENCES warm_intros.messages(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (message_id, message_version_id) REFERENCES warm_intros.message_versions(message_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, context_version_id) REFERENCES warm_intros.context_versions(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, invitation_id) REFERENCES warm_intros.invitations(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** `UNIQUE (delivery_id, attempt_number)`
...
| `introduction_id` | `uuid` · required | Owning introduction. |
| `event_type` | `text` · required | Canonical event name; examples documented below. |
...
**Keys and indexes:** `FOREIGN KEY (introduction_id, participant_id) REFERENCES warm_intros.participants(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, message_id) REFERENCES warm_intros.messages(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, delivery_id) REFERENCES warm_intros.mail_deliveries(introduction_id, id) ON DELETE RESTRICT`; `FOREIGN KEY (introduction_id, context_version_id) REFERENCES warm_intros.context_versions(introduction_id, id) ON DELETE RESTRICT`
...
**Keys and indexes:** Primary key above.
...
| `introduction_id` | `uuid` · nullable | Committed resource, null before a create completes. |
...
**Keys and indexes:** `UNIQUE (scope_type, scope_id, operation, idempotency_key)`
...
- Feature-child foreign keys use RESTRICT. Composite `(introduction_id,id)` and `(message_id,id)` references prevent cross-introduction participant, context, review, version and delivery links.
- A service transaction enforces exactly two distinct participants, distinct normalized email addresses, two request slots, one final slot, different first/second IDs, and correct request recipient identities. No SQL status CHECK is added. Domain validation remains mandatory even with foreign keys.
...
A global context-version change is too broad for review validity: changing Avery’s address should not invalidate Sarah’s unchanged request. Keep the original `context_version_id` on each review as historical evidence, but determine current approval by recomputing `approval_scope_sha256` over canonical relevant inputs:
...
1. Create introduction A with original WHY “Avery is planning Fieldwork’s next phase; Sarah has relevant creative-software experience.” Store that exact paragraph.
...
3. AI produces three versions; sender edits Avery’s note and reviews all three. Retain generated and human versions plus review receipts.
4. Sender starts. Freeze context v2, chosen addresses/order, and the exact three approved message versions.
5. Avery accepts; Sarah declines and leaves a private note. Store both decisions, the displayed disclosure versions, the note, request deliveries/attempts, and neutral closure to Avery.
6. Sender archives A, changes Sarah’s saved address, and later creates introduction B. A still shows the original addresses, WHY, sent messages, decisions and timeline. B links to A as a new attempt and starts with fresh approvals/consent.
...
Add a purpose-specific `suggestionspub.WarmIntroSourceReader` contract for this owner-scoped operation. It returns the two source identities, approved-to-use contact data, delivered draft references, and private drafting context. Keep that contract separate from the existing shared-project reader.

Ask first the person with the sender’s stronger relationship, defined for this feature as the **lower comparable relationship weight**. Use the sender’s own relationship evidence, not an unrelated teammate’s route or an arbitrary shortest-path total. Confirm the authoritative field and its comparison semantics in Phase 0. If evidence is missing, tied, or not comparable, keep stable source order and show a neutral choice without claiming one relationship is closer.
...
Delivery state belongs to each message: queued, sending, provider-accepted, delivered, retryable failure, permanent failure, or unknown outcome. It is separate from consent and the logical sequence stage. A final delivery error does not erase the fact that both accepted or that a message was already sent.
...
| Endpoint | Behavior |
| --- | --- |
| `POST /v1/warm-intros` | Body: source IDs and `action_ref`. Idempotently create or resume the owner’s active draft. Resolve source, recipients, recommendation, and queue three generation jobs. Return persisted draft and generation state. |
| `GET /v1/warm-intros/{id}` | Owner detail including drafts/reviews before launch or frozen communication record afterward. |
| `PATCH /v1/warm-intros/{id}` | Versioned draft edits: order, selected known address, subjects/notes. Apply review invalidation server-side; reject changes after start. |
| `POST /v1/warm-intros/{id}/messages/{message_id}/generate` | Queue generation/retry for one draft. Require the current revision and explicit overwrite intent for human-edited text. Keep existing text until replacement succeeds. |
...
| `POST /v1/public/warm-intros/decision` | Token, invitation version, `accept` or `decline`, optional private note, client idempotency key. Atomically record the decision and next task. Repeating the same decision returns its receipt; a conflicting decision returns the already-recorded outcome. |
...
The public DTO contains introducer display name/avatar, intended recipient first name, other person’s safe profile, the reviewed personal note, invitation state/expiry, and consent disclosure. It excludes email addresses, contact IDs, private WHY, weights, internal events, and the other participant’s token. After a decision, show a minimal receipt; do not turn the link into a contact-directory page.
...
Use real UUIDs in implementation; symbolic IDs below make relationships readable. Owner mutations require both `Idempotency-Key` and `expected_revision` except initial create. Return `409 CONFLICT` with the current resource revision for a stale edit or changed idempotency replay. Authorization runs before returning an old receipt. A version mismatch must never erase the user’s unsaved buffer.
...
Go returns `201` for new or `200` for a resumed active draft. Creation commits the root, exactly two participants, context v1 with full source WHY, three message slots, and three generation runs before answering. Return their IDs immediately; no model output is needed to open the workspace.
...
```json Owner response data — required shape, abbreviated
{
  ...
  "context": {
    "id": "<context UUID>",
    "version": 1,
    "source_why_text": "The original complete paragraph, unchanged.",
    "why_connect": "Avery’s product is entering a phase where Sarah’s experience could be useful.",
    "sender_intent": "I want them to compare notes on the next stage of Fieldwork.",
    "why_status": "present",
    "first_participant_id": "<Avery participant UUID>",
    "second_participant_id": "<Sarah participant UUID>"
  },
  "messages": [
    {"id":"<request Avery UUID>","kind":"request","participant_id":"<Avery participant UUID>","generation_status":"queued","current_version":null,"review":null},
    {"id":"<request Sarah UUID>","kind":"request","participant_id":"<Sarah participant UUID>","generation_status":"queued","current_version":null,"review":null},
...
| Review | `expected_revision`, `message_version_id`, `context_version_id` displayed. Validate, compute approval hash, append review receipt. Do not trust a client `reviewed: true`. |
| Generate/regenerate | `expected_revision`, `message_id`, `base_message_version_id`, `overwrite_human_edits`, optional `revision_request`. Queue one new run; return `202` and its ID. Current text remains until candidate completion. |
...
| Resolve public link | `token`. Return recipient-safe context, invitation version, exact disclosure/version, deadline, and allowed actions. No mutation to consent. |
| Decide | `token`, `invitation_version`, `decision`, `consent_copy_version`, optional private note. Server verifies the shown disclosure version, capability, stage and deadline, then writes the winning receipt and event. Same decision replay returns same result. |
...
Owner history supports `status`, `archived`, `participant_id`, `source_suggestion_id`, `query`, `limit`, and `cursor`. Use `(created_at,id)` for stable newest-first pagination; counts use the same owner/source authorization. Search names and retained WHY/intent without returning an unrelated person’s private source. Start with scoped text search; add a generated search document/GIN index only when measured history size warrants it.

Missing inaccessible rows answer NotFound. Invalid content/address reports typed field validation; missing login is Unauthorized. Expired/decided public links return a controlled recipient-state response after token verification, without exposing internal database IDs. Invalid tokens return a generic unavailable invitation, not whether a person exists.
...
sequenceDiagram
  ...
  par Request to Avery
    W->>L: Stream request prompt (Avery recipient)
  and Request to Sarah
    W->>L: Stream request prompt (Sarah recipient)
...
id: 13
event: draft.delta
data: {"message_id":"request-A","run_id":"run-A1","field":"subject","text":"Avery, meet Sarah?"}

id: 14
event: draft.delta
data: {"message_id":"request-B","run_id":"run-B1","field":"body","text":"Hi Sarah,\n\n"}

id: 15
event: draft.delta
data: {"message_id":"final","run_id":"run-F1","field":"body","text":"Avery, meet Sarah. "}

id: 16
event: draft.completed
data: {"message_id":"request-A","run_id":"run-A1","message_version_id":"A-v1","revision":2,"subject":"Avery, meet Sarah?","body":"Complete validated body here."}
```
...
2. Claim a run with a lease and the message’s active-run pointer. Use worker lifetime, timeout and explicit cancellation for its context; the browser connection does not own the run.
...
9. A crashed provider stream is not resumable at token level. Retry as a new run with a reset/candidate buffer, never append a fresh model response to an old partial. Preserve the failed run and input/output evidence.
10. Commit a completed result only if its active-run pointer, base text revision, and sanitized model-input hash still match. If the sender changed those dependencies, mark the run superseded. An order-only or private-address change does not alter model inputs, so it need not discard useful streaming prose; approval invalidation still applies. Completing a job never marks a draft reviewed or starts sending.
...
Inputs are the sender identity, the two resolved contacts, source WHY, allowed profile facts, and source draft suggestions. Treat source text as data, not instructions. Validate structured subject/body results and length limits; never permit the model to choose addresses, invoke sending, claim consent, or add unsupported personal claims. Do not insert untrusted HTML into email templates.
...
For the public route, put the opaque token in the invitation URL fragment, read it into page memory, and send it to the resolve/decision endpoints in request bodies. Avoid tokens in query strings, access logs, analytics, screenshots, error reports, and referrers. Set `Cache-Control: no-store`, `Referrer-Policy: no-referrer`, and `noindex`; use no third-party trackers on this page. Confirm root providers and auth interceptors do not redirect anonymous recipients or cache their DTOs under authenticated query keys.
...
Proposed limits: three concurrent runs per introduction; six active runs per sender across all instances; 20 new introduction generations per sender per rolling day; six explicit regenerations per introduction per day. Make these server configuration, not UI constants. Reserve capacity/quota atomically. Permit at most one automatic full-run retry per failed message for transient provider failures. Keep it under the same user action but give it a new run ID and clean candidate buffer. No automatic fallback model is configured initially; add one only after the same evaluation corpus passes, and record every fallback’s actual model/run provenance.
...
Persist the full original source context separately from the sanitized model input. Pass only recipient-safe facts and an approved-to-use rationale. Do not send relationship weights, raw private notes, or email addresses to the model merely because they are stored. `sender_intent` remains private by default; when its goal should be shared, the sender’s working `public_why` or an explicitly approved safe fact carries that meaning. No extra model is allowed to infer permission to disclose it.

```json Example user-message payload for Avery’s request
{
  "sender": {"display_name":"Mark Pederson","first_name":"Mark","signature_mode":"first_name"},
  "recipient": {"participant_id":"participant-A","full_name":"Avery Stone","first_name":"Avery","title":"Founder","company":"Fieldwork","safe_bio":"Builds software for independent creative teams."},
  "counterpart": {"participant_id":"participant-B","full_name":"Sarah Chen","first_name":"Sarah","title":"Partner","company":"Northline Ventures","safe_bio":"Works with early-stage creative software founders."},
  "why_connect":"Avery is working through Fieldwork’s next phase. Sarah’s experience supporting creative software founders could make a conversation useful to both of them.",
  "public_why":null,
  "recipient_safe_facts":["Avery is building Fieldwork.","Sarah supports early-stage creative software founders."],
  "source_reference_draft":null,
  "style":{"locale":"en","tone":"personal and concise","request_word_target":"70–130"},
  "revision_request":null
}
```

The second call swaps recipient/counterpart without changing stable IDs. The final call receives `participants: [A, B]`, `why_connect`, `public_why`, allowed facts, and sender/style. It does **not** wait for request-draft output. If only a private sender intent exists, retain it in history and ask the sender to supply a shareable rationale or review a neutral invitation; never pretend a private objective is an externally approved explanation.
...
{
  "from": "Mark via Orbiter <intro@configured-verified-domain>",
...
{
  "from": "Mark via Orbiter <intro@configured-verified-domain>",
  "to": ["avery@example.com", "sarah@example.com"],
...
| --- | --- | --- |
| **1. Confirm schema contracts** | Review all 16 tables, field dictionary, reference SQL, identity mapping, WHY/privacy rules, and history invariants. Confirm PostgreSQL support and existing owner/contact/source FKs against the chosen environment. | Signed-off schema proposal and recorded baseline; no live schema changes as part of documentation. |
| **2. Create actual migrations** | Generate goose migration filenames, create feature schema/tables/indexes/FKs, add SQLC configuration/queries, grant required runtime access. Insert circular pointers only after child rows exist. | Up/down on a disposable empty fixture and up on a representative existing fixture; preserve source rows; verify indexes and runtime permissions. |
...
| **6. Implement consent/state engine** | Capability hashing, resolve allowlist, explicit decisions and note writes, idempotent transitions, next-stage enqueue, decline/cancel/expiry guards and permanent events. | Transaction races prove first-only initial contact and two-consent final prerequisite. Include accept-vs-decline and cancel-vs-dispatch. |
| **7. Implement generation services** | Prompt assets, configured models, three independent run rows, bounded concurrent workers, input hashes, quotas, strict output parser, versions/provenance, retry/supersession. | Three calls run concurrently against fixtures/real staging model; one failure does not discard other completed results. No run can send mail. |
| **8. Implement durable SSE** | Authenticated multiplex endpoint, ordered stream event persistence, replay/snapshot reset, heartbeat, proxy settings, reconnect, explicit cancel. | Close/reopen browser mid-generation and recover all three results without a duplicate model job; stale deltas never corrupt edited text. |
| **9. Build the full-screen setup** | Leverage Loop action mapping, known people/WHY, three live composers, generation/error states, recipient-based editing, order switching, autosave/conflict states, previews and review/start UI. | Browser-to-DB assertions for each save/review; starts only the exact visible saved version. Non-Warm-Intro actions remain intact. |
| **10. Build public mobile route** | Unauthenticated invite shell, token-in-fragment resolver, safe profile/note, exact consent disclosures, accept/decline/private-note and all terminal/error states. | Separate clean browser sessions accept in sequence; no login requirement or shared authenticated data cache. Scanner GETs cause no consent. |
| **11. Add email renderer and Resend client support** | Four HTML/text templates, server preview endpoints, verified From/Reply-To/Cc, deterministic payload freezing, encryption and tags, typed provider errors. | Fixture rendering passes privacy/escaping checks; exact request bytes/hash survive retries. Existing account-review sends still pass. |
...
| **13. Build tracking modal and detail** | All/pending/introduced/declined/expired/cancelled/needs-attention views, timestamps, WHY/intent, original vs reviewed context, sent messages, consent steps, delivery attempts and cancellation. | Every synthetic scenario has a complete readable timeline; archive changes visibility only. Owner history never exposes another sender’s introductions. |
