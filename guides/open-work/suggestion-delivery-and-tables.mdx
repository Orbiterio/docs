...
[Add Required Entities](/guides/open-work/suggestion-delivery-and-tables/add-required-entities) lists the eight people and four companies/organizations whose entity IDs are needed for the ready and placeholder suggestions. Six identity links have been supplied; six remain to be confirmed.
...
| Suggestion `content_status` | `ready` or `placeholder`. Describes whether supporting content is supplied; independent of terminal batch status, dismissal, and accordion state. |
...
| Suggestion | `source_ref`, `rank`, `entity_id`, `entity_type`, `content_status`, `suggestion_mode`, `badge`, `why`, `score`, `trajectories[]`, `suggested_sequencing[]`, `key_people`, `hooks`, `paths[]`, `evidence[]`, `entities[]`, `action_groups[]`, `actions[]` |
| WHY | For `ready`: nonempty `headline` string and nonempty `body[]` of nonempty paragraph strings. For `placeholder`: null. Preserve paragraph boundaries. |
...
A `placeholder` is a named candidate with supporting content not yet supplied. Preserve its `source_ref`, rank, typed target, and `entities[]` registry, including the beneficiary for Leverage Loop and any `target_org` used in its header. Require `why`, `badge`, `key_people`, and `hooks` to be null, `score` to be `{}`, and `trajectories`, `suggested_sequencing`, `paths`, `evidence`, `action_groups`, and `actions` to be empty arrays. Store `why: null` as SQL NULL. Placeholder rows count toward `suggestion_count`; do not create empty child rows for those arrays. The batch's `complete` status means delivery is final, not that every candidate has ready content. A later populated version arrives in a new immutable batch with a new batch ID.
...
| --- | --- |
| Empty named suggestion bars | `suggestion.content_status = placeholder`, rank, target, and typed `suggestion_entity` rows. Hydrate names; expanding shows an empty content area with no generated WHY or action controls. |
...
| `suggestion` | One row per candidate within a batch, including placeholders. Typed entity, content status, badge, nullable structured WHY, score, key people, hooks, and reserved dismissal fields. No aggregate trajectory columns. | POST mechanism |
...
The Leverage Loop sample creates three suggestions; the Outcome sample creates four. For Leverage Loop:
...
Within a visible batch, return both ready and placeholder suggestions in rank order. Preserve null WHY and empty arrays on reads; do not drop placeholder cards or treat them as archived. The design's `ARCHIVED 1` counter has no corresponding supplied record and does not add a row to these fixtures.
...
7. Require `content_status` to be `ready` or `placeholder`. Ready suggestions require a nonempty `why.headline` and nonempty `why.body[]` of nonempty paragraphs; preserve text. Placeholders require the null/empty content shape in the envelope contract while retaining normal entity and rank validation. Ready suggestions may also have empty scores, paths, and evidence. Trajectories, sequencing, and non-null key people are Outcome-family fields; Leverage Loop uses empty trajectories/sequencing and null key people. `hooks` may appear on any ready suggestion type, each with `entity_id`, `entity_type`, `angle`, and `text`. Nonempty path hops carry entity type and match the declared `hop_count`.
...
Leverage Loop has one ready suggestion and two placeholders; Outcome has one ready suggestion and three placeholders. The seven suggestions reference twelve unique entities, with fourteen entity/role rows because Katelyn appears in all three Leverage Loop registries. Placeholder suggestions create no action, group, trajectory, sequencing, path, or evidence rows.
...
| Five placeholder suggestions | Persist alongside the two ready suggestions; return their ranks, entity registries, null WHY, and empty content arrays. Counts are 3 for Leverage Loop and 4 for Outcome. |
| Missing/unknown `content_status`, ready with null/empty WHY, or placeholder with populated content | 400 with a field path; no result rows. |
...
| Named empty suggestions omitted / WHY always required | Persist `content_status` and nullable WHY; validate ready and placeholder content separately |
