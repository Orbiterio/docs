...
| `media_ingest` | Validate/ingest a declared artwork candidate; resize and publish through the shared media pipeline | Search galleries, traverse linked games, admit roots or reset network/byte budgets |
...
| --- | --- | --- |
| Existing IMDb title snapshot/media asset | The game title's already acquired poster, when present | Reuse the existing media reference and provenance for the same canonical game; no extra IMDb crawl. Never inherit the poster of its movie adaptation. |
...
Artwork is an optional later stage in the **same finite plan**: at most three ranked candidates and one selected asset per game. Source metadata requests, downloads and retries count toward the existing root network/byte budgets; no gallery enumeration or separate budget. One bounded `media_ingest` job may validate and resize an approved candidate through the existing Go media pipeline, write a versioned GCS object, then publish the shared `film_tv_display` image field. It has no discovery capability and cannot launch another waterfall. A missing, rejected or over-budget image publishes the deterministic placeholder and leaves credits/relationships usable.
...
| Video_Game → `PUBLISHED_BY` → Company | 45, new | Named publisher; preserve territory, platform and dates in scoped records. Distribution is not silently publishing. | Core |
...
Log `root_job_id`, upstream lineage, job kind, game UUID, attempt, lease fence, source revision, policy version, selected/deferred counts, refusal reason and stage outcome. Metrics separate true failures from ordinary pending work: root spend, queue age, saturation, unresolved identities, elected-vs-materialized relations, graph/projection lag, lease reclaims, stale-generation skips, duplicate admissions, blocked recursion and source coverage.
