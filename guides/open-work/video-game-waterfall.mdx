...
| Non-replay work jobs | 100 per root | Includes discovery, game, leaf and description jobs; no budget reset on retry or cross-waterfall handoff |
| Network attempts | 100 per root | Includes searches, page fetches, profile lookups, LLM attempts and retries; separate provider quota also applies |
