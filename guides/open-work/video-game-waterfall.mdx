...
| 2 | `tagline` | `z-ai/glm-5.3-flash` over the description | `reasoning.effort: low` — a bounded level; never `enabled: true` (unbounded thinking blows the budget) or `enabled: false` (400, reasoning is mandatory for this model). `provider.sort: throughput`; `moonshotai/kimi-k3` pinned failover on empty content. Prompt targets **10–14 words**, not a character count (models ignore character counts); **≤112 characters enforced in code** at a word boundary. **Do not restate the release year** — the card renders `first_release_year` as its own badge, so a year in the tagline reads twice. Model, effort, character count, trim flag and timestamp are stage telemetry — `log_crash.phase = tagline`, already in the enum — not node properties. ≈\$0.00013 per game. |
...
| 4 | `embeddings` | `"{name}: {description} Tags: {tags, comma-joined}"` | Gemini `gemini-embedding-001`. **3072** in AlloyDB per the locked vector standard; prefix-truncated and renormalized to **1536** if projected into FalkorDB `Entity.embeddings`, the shape of the 2026-07-06 migration. Appending the tags is deliberate: it makes "competitive esports shooter"-style queries land on the right games where the description alone would not. |
...
**Worked example — Counter-Strike 2** (real values from Wikidata `Q111165107` and Steam app `730`, observed 2026-09-09; the tagline is a simulated placeholder pending a live run):
...
```json
{
  ...
  "description_source": "generated_from_elected_facts",
  ...
  "tagline": "Valve's free-to-play first-person shooter and successor to Counter-Strike: Global Offensive",
...
<div style={{display:'flex', gap:'16px', flexWrap:'wrap', margin:'16px 0'}}>
  <div style={{display:'flex', gap:'14px', width:'384px', padding:'12px', borderRadius:'12px', background:'#1c1c1e', color:'#f2f2f2', fontFamily:'system-ui, -apple-system, sans-serif', boxSizing:'border-box'}}>
    ...
    <div style={{flex:1, minWidth:0, display:'flex', flexDirection:'column'}}>
      ...
      <p style={{margin:'4px 0 8px', fontSize:'13px', lineHeight:1.35}}>FromSoftware's action role-playing game, published by Bandai Namco Entertainment.</p>
  ...
  <div style={{display:'flex', gap:'14px', width:'384px', padding:'12px', borderRadius:'12px', background:'#1c1c1e', color:'#f2f2f2', fontFamily:'system-ui, -apple-system, sans-serif', boxSizing:'border-box'}}>
    ...
    <div style={{flex:1, minWidth:0, display:'flex', flexDirection:'column'}}>
      ...
      <p style={{margin:'4px 0 8px', fontSize:'13px', lineHeight:1.35}}>Valve's free-to-play first-person shooter and successor to Counter-Strike: Global Offensive.</p>
      <div style={{display:'flex', flexWrap:'wrap', gap:'6px', marginBottom:'8px'}}>
...
The taglines shown are the simulated placeholders pending a live GLM run; the portraits are the real Steam assets loaded directly from the CDN, so this block renders the same source files the pipeline will ingest.
