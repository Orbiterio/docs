...
[The Game Awards adapter and additional source observations](/guides/open-work/video-game-waterfall#the-game-awards-adapter) extend the source plan without adding nodes to this core fixture. Its existing 2022 award keeps the same UUID and now has an outcome-independent nominee-entry key.
...
This page contains selected evidence-backed design examples, not a passing executable test suite. Follow [phase 0](/guides/open-work/video-game-waterfall-build-guide#phase-0-freeze-contracts-and-build-the-eval-harness) to promote them into runtime fixtures. Several older core edges still lack newer required scope-reference fields; companion/core rows can also have earlier envelope fields. Add correlated records only from the retained exact source, and validate conditional properties against endpoint type. `development_scopes` is not required on a game-to-company edge, `release_entity_ids` is not required on direct availability, and an engine version is optional. Never fill a missing source relationship with an invented claim.

All fifteen edge families need explicit positive/reject/replay/correction cases, including those absent from this 29/30 sample. Clearly synthetic control-flow cases can test a writer without pretending to supply real provider evidence. Combined fixture counts are not the expected output of every root. The [machine-readable gates](/assets/video-game-waterfall/build-evals.json) remain `not_run` until the Go builder produces actual receipts.

## Game thumbnail example

**Elden Ring — real Steam library portrait.** The game already has Steam app ID `1245620`; no title search or recursive lookup is needed to locate its exact store page. This is the locked 2:3 portrait candidate (`library_600x900.jpg`, measured 300×450), not a simulated or generated cover. The landscape store header remains a letterboxed fallback only.

<img
  src="https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/1245620/library_600x900.jpg"
  alt="Elden Ring official Steam library portrait artwork, 300×450"
  width="300"
/>

[Official store page](https://store.steampowered.com/app/1245620/_/) · [Source image](https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/1245620/library_600x900.jpg) · [Landscape fallback](https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/1245620/header.jpg?t=1787868578) · [Thumbnail source and pipeline plan](/guides/open-work/video-game-waterfall#game-thumbnails-and-cover-art)

The following is a **proposed candidate record**, using the observed source URL and real game IDs. No image has been copied to Orbiter's GCS, elected into a production display row or declared licensed by this example. The observed dimensions were measured from the fetched file on 2026-09-09; internal media IDs remain absent until the media worker ingests it. The planned storage key uses the fixture's simulated game UUID and the shared file pipeline's actual source-URL hash algorithm. This supporting record does not change the fixture's 29-node/30-edge count.
...
```json
{
  ...
  "source_image_url": "https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/1245620/library_600x900.jpg",
  "asset_kind": "steam_library_portrait",
  "orientation": "portrait",
  "observed_width": 300,
  "observed_height": 450,
  "aspect": "2:3",
  "observed_bytes": 52021,
  "storage_prefix": "game-thumbnail/",
  "planned_storage_key": "game-thumbnail/8d718f08-fad1-5f49-9b39-a3ed7678612b/<source-url-hash>.webp",
  "status": "candidate",
...
All candidates use the **same file pipeline as avatars and movie posters**, with the movie-poster aspect-preserving conversion and the `game-thumbnail/` destination. On successful ingestion, persist the returned storage key and serve it through the shared media renderer. Every game card uses the locked 2:3 frame; the Steam library portrait is the primary source and any other candidate is contained within that frame rather than served at its own ratio. IMDb artwork, including already rehosted copies, is excluded. A game stays hidden until the game flow has prepared its properties, ingested approved artwork and committed publication. The planned storage key's hash segment is computed by the shared pipeline from the selected source URL at ingest; it changed with the source and is left symbolic here rather than invented.
...
<img
  ...
  alt="Hamlet game portrait from its official Steam listing, 300×450"
  width="300"
/>

[Official store page](https://store.steampowered.com/app/222160/) · [Source image](https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/222160/library_600x900.jpg) (300×450, 2:3, measured 2026-09-09) · [Landscape fallback](https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/222160/header.jpg?t=1765636960). This artwork belongs to the game's Steam listing; its store-release date does not replace the original-game date in the fixture. It uses the same `game-thumbnail/` pipeline contract. Both images are source candidates, not claimed uploaded GCS objects.
...
**Availability fixture corrected September 9:** each edge now references its exact P400 platform statement and a correlated availability record with reviewed official release evidence: [Steam's Windows release](https://store.steampowered.com/app/1245620/ELDEN_RING/) and [Bandai Namco's PS4 listing](https://en.bandainamcoent.eu/elden-ring/elden-ring). The earlier copied P179 series-membership reference was invalid evidence for availability. [Authoritative AVAILABLE_ON contract](/guides/ontology/edges#available-on). Platform nodes also carry the [locked short description and official website](/guides/ontology/nodes#game-platform-properties); there is no standalone platform card.
...
| Elden Ring → **AVAILABLE_ON** → Microsoft Windows | 80 | Required `availability_record_ids`: exact Windows P400 + official released Windows evidence |
| Elden Ring → **AVAILABLE_ON** → PlayStation 4 | 80 | Required `availability_record_ids`: exact PS4 P400 + official released PS4 evidence |
