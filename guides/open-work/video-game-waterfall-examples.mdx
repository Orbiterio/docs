...
The following is a **proposed candidate record**, using the observed source URL and real game IDs. No image has been copied to Orbiter's GCS, elected into a production display row or declared licensed by this example. Dimensions and internal media IDs remain absent until the media worker inspects/ingests it. This supporting record does not change the fixture's 29-node/30-edge count.
...
For a portrait card, use the exact game's existing IMDb poster or the enabled IGDB/Moby cover source. Preserve the header's aspect ratio when using this Steam candidate; do not imply that its landscape artwork is a portrait cover.
