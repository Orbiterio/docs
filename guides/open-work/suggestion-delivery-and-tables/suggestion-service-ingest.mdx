...
<Warning>
**Built on a branch, not merged or deployed.** Steps 1–6 are on the `orbiter-backend` branch `feat/suggestion-hooks` (commits `6325e62a`, `c0274364`, `2cee69b2`, `01d6f815`, `22c29d4e`, `f113f7f8`) and were verified against a local database on 2026-09-24. `main` and dev still behave as [What happens on main](#what-happens-on-main) describes, and the rest of this page describes `main`. Deploying waits on Universe sending `ref`: see [Decisions](#decisions).
</Warning>
...
| `max_tokens` | No | integer | Only on generated hooks. Greater than `0` and at most 2147483647 (the `INT` column) when present |
...
Migration `migrations/20260923224419_suggestions_005_hooks.sql` on the branch; it must be re-timestamped to be newer than `main` before it merges. It follows `suggestion_action` for the `ref` key and `suggestion_node` for `position`, because hooks have no rank.

The migration also backfills hooks already stored as JSONB. Each gets `ref` `hook-<position>`, written into `suggestion.hooks` first and then copied into the table, so the two agree; hooks that already have a `ref` keep it. Down drops the table and leaves the added keys, which older code ignores.
...
| 6 | `validate_refs.go` | `validateHooks` checks `ref` with the existing `refs` helper (`suggestions[i].hooks[j].ref`: `required` or `duplicate`); `validateHookRecipe` adds the recipe rules from the shape table, plus `max_tokens` `too large` above the 32-bit `INT` column. The existing registry, `angle`, and `text` checks stay |
| 7 | `repo_ingest_rows.go` | `insertSuggestion` calls `insertHooks`, which writes one `suggestion_hook` row per hook in the same transaction, after `suggestion_node` |
...
| Backfill `suggestion_hook` from hooks already stored as JSONB? | **Backfill in the migration.** The local count found 8 hooks on 4 suggestions from one real Universe loop, which shows Universe already sends hooks without refs (angles `mechanism` and `rationale`). The migration writes `hook-<position>` into the JSONB and the table. Dev and staging have not been counted |
| Write the backfilled refs into the JSONB too? | **Yes, both.** The table and the JSONB stay identical, so the API sees a ref on every hook |

### Order of work

1. **Open:** agree the `ref` addition with Universe. Until Universe sends it, every delivered loop with hooks would be rejected.
2. **Done:** migration, queries, and `sqlc` generation (`6325e62a`).
3. **Done:** wire and domain structs, including the `Hook.Ref` → `Entity` rename, and mapping (`c0274364`); validation (`2cee69b2`).
4. **Done:** the table insert and the JSONB write, with the round-trip integration test (`01d6f815`).
5. **Done:** the CLI report count (`22c29d4e`).
6. **Partly done:** the Angel Outcome Demo package carries refs, and the Go acceptance test passes (`f113f7f8`). The same package loaded into a local database on 2026-09-24 with every expected row. The dev load waits on the merge and deploy.
7. **Done:** these docs.
...
### `suggestions.suggestion_hook` (branch only)

On `feat/suggestion-hooks`, one row per `hooks[]` item (`CreateSuggestionHook`, written by `insertHooks`): `ref`, `position` (the 1-based array index), `entity_type`, `node_uuid`, `angle`, `text`, and the nullable recipe columns `model`, `tone_instruction`, `temperature`, `max_tokens`. Unique on `(suggestion_id, ref)` and `(suggestion_id, position)`; deleted with its suggestion. The JSONB `suggestion.hooks` is still written and is what `ReadBatch` and the feed read; the rows must equal it element by element. See [Scope for hooks](#scope-for-hooks).
...
| --- | --- | --- |
| `draft_email` | `{emails[], draft: {subject, message}, sender_name?}` | Required; a declared person |
