---
title: "Calendar View — Frontend Brief"
sidebarTitle: "Frontend brief"
description: "The supplied frontend contract: product frame, API types, collections, Temporal-based UI, performance budgets and guardrails."
---

[Calendar View](/guides/open-work/roadmap/calendar-view) · [API integration](/guides/open-work/roadmap/calendar-view/api-contract) · [Phase 0](/guides/open-work/roadmap/calendar-view/frontend/phase-0)

<Note>
The numbered sections below preserve sections 1–6 of the supplied frontend brief. The implementation handoff destination is `orbiter-frontend/docs/calendar/BRIEF.md`; the [downloadable BRIEF.md text](/assets/calendar-view/BRIEF.md.txt) contains those sections, and the [handoff ZIP](/assets/calendar-view/calendar-view-handoff.zip) includes a correctly named Markdown file. Reference it from the frontend repository's contributor instructions during implementation.
</Note>

Read the [integration gates](/guides/open-work/roadmap/calendar-view/integration-gates) alongside the fixed contract. They identify missing API semantics and reference-code corrections without silently adding fields. In particular, all-day UTC midnights encode dates, native EventSource 410 handling needs a defined protocol, mutation request bodies remain unspecified, and participant information uses flat rows under the no-cards design direction.

Read the [QA findings](/guides/open-work/roadmap/calendar-view/qa-review) and [Akira JEV addendum](/guides/open-work/roadmap/akira/jev) with every phase prompt. The bundle supplies `QA-REVIEW.md`, `INTEGRATION-GATES.md` and `JEV.md` alongside `BRIEF.md`. The original sections below stay unchanged; the addenda identify the existing API/identity migration, missing dependency decisions and two optional pilots. No JEV call belongs in the sync or render path.

## Navigation and cache amendment

Read [Navigation and shared cache](/guides/open-work/roadmap/calendar-view/navigation-and-cache) (`docs/calendar/NAVIGATION-AND-CACHE.md` in the handoff) before the preserved source sections. Add Cmd-K → Navigation → Calendar View, top-left icon navigation, `0 0` for the full Calendar View and `0` for today's agenda. One shortcut controller resolves the overlapping sequence.

The proposed 60-day forward warm range is a minimum, combined with the original visible-range margins and edge extension. All views and the existing widget must consume one shared, all-day-capable store updated from the mirror's change stream. The current 14-day widget projection is not complete coverage. This amendment takes precedence over the source's preload assumptions; its original sections remain unchanged below.

Also read [Agenda and scheduling](/guides/open-work/roadmap/calendar-view/agenda-and-scheduling) (`AGENDA-AND-SCHEDULING.md`). Its no-scroll month paging replaces the original infinite-month/30-screen-scroll requirement. The agenda List uses meeting context, paired with a visual Timeline. The shared scheduler accepts contact search or direct emails and creates real provider events/invitations through Nylas after the write gates pass. Draft editing and picker Apply do not send invitations.

Read [Performance requirements](/guides/open-work/roadmap/calendar-view/performance) (`PERFORMANCE.md`) before every phase. It defines the effective nine-slot matrix, P01–P08, the measurement environment and the agent's reporting obligations. Its month-paging gate replaces source slot 5; the other eight original limits remain. All numbers are targets to prove during implementation.

The [active-account amendment](/guides/open-work/roadmap/calendar-view/navigation-and-cache#active-calendar-account-in-the-left-panel) adds the left-panel Calendar account selector and per-account New events in destination. It supersedes any unscoped primary-calendar fallback below. Switching shows only that account's enabled calendar events and defaults new drafts to it; existing drafts/events retain their reviewed identity. FREE / OPEN time checks all calendars selected for availability across accounts, independently of display selection.
