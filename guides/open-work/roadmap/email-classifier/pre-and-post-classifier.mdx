...
<Danger>
**Not Ready for Dev yet - still scoping.** Companion to the [Email Classifier](/guides/open-work/roadmap/email-classifier) overview. Phases marked *exists* are live in `orbiter-backend` log-and-parse today; everything else is design.
</Danger>
...
**Redact and cap.** URL query strings and tokens of 32+ characters are removed; the text is cut at 2,500 characters, head first. AI-off grants stop here with no body: they get a rules-only classification.
...
| Phase | Actor | Reads | Writes | Runs on AI-off grants |
| --- | --- | --- | --- | --- |
| 0 Sweep and stage | code (exists) | Nylas messages | `import_classify_queue`, `import_email_message` | yes, without body |
| 1 Prep | code | queue row, sender rules, contact tables | in-memory envelope | yes, without body |
| 2 Classify | Jev | envelope | `Result` | rules-only (`classifier.Rules`) |
| 3a Store non-human | code | `Result` | `non_human_email_activity`; receipt file via files pipeline | yes |
| 3b Promote | code (exists) | staged rows | contact, `email_activity`, interaction log | yes |
