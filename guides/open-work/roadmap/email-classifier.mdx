...
task, signals (no body text), answers (probabilities), usage and latency.
...
- `internal/features/emails/classifier`: `LoadAsset`, `New`, `Classify`,
  `Rules`, `DefaultOptions`; signals, candidates, request builder, composition;
  14 unit tests (`go test ./internal/features/emails/classifier/`), lint clean.
- `internal/clients/openrouter/decisions.go`: `noul` range validation + tests.
- `assets/prompts/email-classifier.md` version 2026-09-22.4: 20 questions, one
