...
<a id="theatre-graph-output-manifest" />

### Closed graph output manifest — September 7 sync

**Node Types and Edge Types are the canonical property references for this build.** The following checklist exhausts theatre-owned output families in this plan. Read each linked definition and its complete synthetic JSON example before implementing a DTO/writer. Examples are contract fixtures, not permission to ingest a source or claims about live graph data. Canonical SQL/API type names and graph-family `entity_type` values are explicitly mapped in Node Types.

| Node family | Scope / creation rule | Definition and complete example |
| --- | --- | --- |
| `Entity:Stage_Production` with optional `Play`/`Musical` | Required v1 hub; canonical `stage_production` admission | [Properties](/guides/ontology/nodes#stage-production) · [Example](/guides/ontology/nodes#stage-production-example) |
| `Entity:Work:Theatrical_Work` with optional `Play`/`Musical` | Optional resolved-work slice; canonical `theatrical_work` admission | [Properties](/guides/ontology/nodes#theatrical-work) · [Example](/guides/ontology/nodes#theatrical-work-example) |
| `Place:Venue:Theatre` | Optional support-place extension; preserve existing owner/UUID | [Properties](/guides/ontology/nodes#theatre-venue) · [Example](/guides/ontology/nodes#theatre-venue-example) |
| `Stage_Award` with current `Winner`/`Nominee` modifier | Later support-result slice; one durable scoped entry | [Properties](/guides/ontology/nodes#stage-award) · [Winner/nominee examples](/guides/ontology/nodes#stage-award-example) |
| Existing `Person` / `Company` | Reused in the initial pilot; later creation only through owner gates | [Existing endpoint examples and ownership](/guides/ontology/nodes#theatre-reused-endpoints) |
| Existing `Music_Group` / `Film_TV` | Optional creator/source-work endpoints only; no theatre-driven creation or cascade | [Existing endpoint examples and ownership](/guides/ontology/nodes#theatre-reused-endpoints) |

| Relationship / exact endpoints | Scope | Canonical properties and complete example |
| --- | --- | --- |
| `Person/Company → PRODUCED → Stage_Production` | Required v1, 12/16/22/28/35 by evidenced tier | [PRODUCED](/guides/ontology/edges#stage-produced) · [Private company submission](/guides/ontology/edges#stage-produced-private-example) |
| `Stage_Production → PRODUCTION_OF → Theatrical_Work` | Optional work context; 50, excluded | [PRODUCTION_OF](/guides/ontology/edges#stage-production-of) |
| `Stage_Production → STAGED_AT → Place:Venue:Theatre` | Optional venue context; 50, excluded | [STAGED_AT](/guides/ontology/edges#stage-staged-at) |
| `Theatrical_Work → ADAPTED_FROM → Film_TV/Theatrical_Work` | Later explicit work lineage; 55, excluded | [ADAPTED_FROM](/guides/ontology/edges#stage-adapted-from) |
| `Person/Music_Group → WROTE → Theatrical_Work` | Optional work authorship; 10, excluded | [WROTE](/guides/ontology/edges#stage-wrote) |
| `Person/Music_Group → COMPOSED → Theatrical_Work` | Optional work authorship; 8, excluded | [COMPOSED](/guides/ontology/edges#stage-composed) |
| `Person/Music_Group → WROTE_LYRICS → Theatrical_Work` | Optional work authorship; 12, excluded | [WROTE_LYRICS](/guides/ontology/edges#stage-wrote-lyrics) |
| `Stage_Production → HAS_AWARD → Stage_Award` | Later scoped result context; 40, excluded | [HAS_AWARD](/guides/ontology/edges#stage-has-award) |
| `Person/Company → WON_AWARD → Stage_Award:Winner` | Later explicitly named recipient; 30, excluded | [WON_AWARD](/guides/ontology/edges#stage-won-award) |
| `Person/Company → NOMINATED_FOR → Stage_Award:Nominee` | Later explicitly named recipient; 35, excluded | [NOMINATED_FOR](/guides/ontology/edges#stage-nominated-for) |

Every edge has `description`, `description_generated_at`, `active`, source/evidence references, revision and the appropriate election/support-claim reference. Direct credits also have the explicit `stage_credit_version` fence; no generic Tony-list heading earns a tier by default. Node/edge public URLs are conditional on actual public evidence; private submission references must not be turned into fake public URLs. Existing person/company/music/film metadata and shared-place ownership remain authoritative for reused endpoints.

Read-only dependencies are not extra stage outputs: expertise taxonomy drives intake, and company/filing affiliations can support review or an existing traversal. The stage owner does not emit `HAS_EXPERTISE`, `OFFICER_OF`, `BOARD_MEMBER_OF`, PSC/ownership edges, movie/music credits, or any new principal-credit shortcut. In particular, the 990 officer edge contract cannot be repurposed for Companies House roles without an owner-defined extension. Cast/directing/performance/venue-ownership/tour links, book nodes and `CO_PRODUCED_WITH` remain outside this build. Source jobs, snapshots, billed groups, mentions, resolution decisions and credit/checkpoint ledgers stay in SQL/GCS; they are not additional graph nodes. Any added output family must first extend these Graphs definitions, examples, endpoint allowlists and acceptance fixtures.
...
| Source-specific clients/adapters: BroadwayWorld evaluation, reviewed programme/submission, Wikidata/Tony, separately enabled IBDB/Playbill/UK sources | Fetch/decode only enabled sources; retries through `platform/httpx`; no entity or graph writes |
...
- Raw source provider names are distinct (`broadwayworld`, `wikidata`, `wikipedia`, `tony`, `pilot_submission`, reviewed programme publishers, and separately enabled `ibdb`/`playbill`/UK sources). A derived extraction/resolution envelope identifies its actual provider, snapshot, parser and policy; it never masquerades as IMDb or MusicBrainz.
...
| Property | Rule |
| --- | --- |
| `entity_type`, `form` | SQL/API canonical type `stage_production`; graph `entity_type = Stage_Production` and label `Stage_Production`, following the current Go family-name convention. `Theatrical_Work` similarly maps to SQL/API `theatrical_work`. Form `play/musical/other`, unknown absent. No `Film_TV` label or second `display_label` graph discriminator. |
...
| `ibdb_production_id`, `broadwayworld_show_id`, `wikidata_id`, `source_url`, `source_names` | Verified engagement-scoped aliases and evidence summary from Node Types. A work-level QID/IBDB show ID belongs on the work; public URLs are conditional for submission-only evidence. The typed registry remains authoritative. |
...
Require stable credit/edge UUID, producer/production canonical IDs, chosen fact ID, `mention_ref`, billing tier, original source role, 1-based billed-group order, market, actual source names and immutable evidence references, permitted public source URLs when available, numeric `identity_confidence`, provenance/version, description and timestamps. Submission-only pilot evidence may omit public URLs. Require `active` and `stage_credit_version = projection_revision` for the current desired credit version. Group/member identity stays available through `credit_group_id` and `member_order`; multiple supporting mention IDs remain in SQL. Description: `{Producer} is credited as {source role} on {Title} ({market}, {season}).`
